...
int main(int argc, char *argv[]) {
  ...
  if (argc < 5) {
    cerr << "usage SkimNtuple inputList.txt <outputDirectory> "
            "<outputfileLabel> <Skim Cut String>"
         << endl;
    return -1;
  }
  ...
  for (auto &line : inputLines) {
    ...
    // if the first key is a TDirectoryFile, go inside it and skim there
    // (temporary hack for cloning a single directory)
    ...
    while ((key = (TKey *)nextkey())) {
      ...
      if (className.find("TH1") != string::npos) {
        ...
        cout << "Copying histogram " << outHist->GetName()
      ...
      TTreeFormula *formula =
          new TTreeFormula("SkimCutString", SkimCutString.c_str(), inputTree);
      int EventsPassed = 0;
      ...
      delete formula;
      cout << "Skim Efficiency : " << EventsPassed << " / "
