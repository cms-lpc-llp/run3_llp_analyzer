...
/////////////////////////////////////////////////////////////////////////
SimpleJetCorrectionUncertainty::SimpleJetCorrectionUncertainty(
    const JetCorrectorParameters &fParameters) {
...
/////////////////////////////////////////////////////////////////////////
float SimpleJetCorrectionUncertainty::uncertaintyBin(unsigned fBin, float fY,
                                                     bool fDirection) const {
  if (fBin >= mParameters->size())
    std::cout << " wrong bin: " << fBin << ": only " << mParameters->size()
              << " are available" << std::endl;
  const std::vector<float> &p = mParameters->record(fBin).parameters();
  if ((p.size() % 3) != 0)
    std::cout << "wrong # of parameters: multiple of 3 expected, " << p.size()
              << " got" << std::endl;
...
/////////////////////////////////////////////////////////////////////////
float SimpleJetCorrectionUncertainty::linearInterpolation(
    float fZ, const float fX[2], const float fY[2]) const {
  // Linear interpolation through the points (x[i],y[i]). First find the line
  // that is defined by the points and then calculate the y(z).
  float r = 0;
  if (fX[0] == fX[1]) {
    if (fY[0] == fY[1])
      r = fY[0];
