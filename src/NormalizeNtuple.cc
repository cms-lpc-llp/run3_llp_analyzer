...
// Get total number of events in the sample and determine normalization weight
// factor
double getNormalizationWeight(string filename, string datasetName,
  ...
  string pathname;
  if (cmsswPath != NULL)
    pathname = string(cmsswPath) + "/data/xSections.dat";
...
int main(int argc, char *argv[]) {
  ...
  if (argc < 2) {
    cerr << "usage NormalizeNtuple inputList.txt <integrated lumi in /pb>"
         << endl;
    return -1;
  }
  ...
  while (getline(filein, curFilename)) {
    if (curFilename.at(0) != '#')
      ...
      cout << "(Skipping commented line in input)" << endl;
  }
  ...
  if (_randomSeed != "")
    randomSeed = atoi(_randomSeed.c_str());
  if (doUnweight)
  ...
  for (auto &line : inputLines) {
    ...
    double normalizationWeight =
        getNormalizationWeight(fileName, datasetName, intLumi);

    // create output file
    TFile *outputFile = new TFile(
        Form("%s_%.0fpb_weighted.root",
             (fileName.substr(0, fileName.find_last_of("."))).c_str(), intLumi),
        "RECREATE");
    cout << Form("%s_%.0fpb_weighted.root",
                 (fileName.substr(0, fileName.find_last_of("."))).c_str(),
                 intLumi)
         << endl;
    // loop over all TTrees in the file and add the weight branch to each of
    // them
    ...
    while ((key = (TKey *)nextkey())) {
      ...
      // if this key has the same name as the previous one, it's an unwanted
      // cycle and we skip it
      if (previous != NULL &&
          strcmp(key->GetName(), previous->GetName()) == 0) {
        continue;
      }
      previous = key;
      ...
      for (int n = 0; n < inputTree->GetEntries(); n++) {
        ...
        if (normalizationWeight >= 0) {
          if (foundWeightBranch) {
            // if weight branch exists, then multiply the value stored by the
            // normalizationWeight
            weight = inputweight * normalizationWeight;
            // weight = normalizationWeight;
          } else {
            // if the weight branch doesn't exist, use the normalizationWeight
            // as the weight
            weight = normalizationWeight;
          }
        }
        ...
        if (!doUnweight) {
          ...
          if (randomNum < (normalizationWeight)) {
            // cout << normalizationWeight << " : " << 1 / (normalizationWeight)
            // << " " << weight << " -> ";
            weight = weight / (normalizationWeight);

            // apply some filter cuts
            TTreeFormula *formula = new TTreeFormula(
                "SkimCutString", "MR>300 && Rsq>0.15", normalizedTree);
            bool passSkim = false;
