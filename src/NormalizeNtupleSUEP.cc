...
double getNormalizationWeight(string filename, string datasetName, double intLumi) {
  ...
  TH1F *hist = (TH1F*) file->Get("ntuples/NEventsGen");
  //TH1F *hist = (TH1F*) file->Get("NEvents_genweight");
  if (!hist) {
    cout << "Could not find histogram NEventsGen"
  ...
  string pathname;
  if(cmsswPath != NULL) pathname = string(cmsswPath) + "/data/xSectionsSUEP.dat";
  
  SimpleTable xstab("data/xSectionsSUEP.dat");
...
int main(int argc, char* argv[]) {
	...
	string mh_substring = line.substr(line.find("mMed-")+5);
	string mh = (mh_substring.substr(0,mh_substring.find('_')));
        string datasetName("mMed-");
        string fileName(line);
	datasetName += mh;
	std::cout << "datasetName: " << datasetName << std::endl;
	std::cout << "fileName: " << fileName << std::endl;
	string outDir = (fileName.substr(0, fileName.find_last_of("/"))) + "/weighted/";
	string outFileString = outDir+(fileName.substr(fileName.find_last_of("/")+1,-1));
	...
	TFile *outputFile = new TFile(outFileString.c_str(), "RECREATE");
        //loop over all TTrees in the file and add the weight branch to each of them
	TFile *inputFile = TFile::Open(fileName.c_str(), "READ");
	cout << "input: " << fileName << "\n";
	cout << "output: " << outFileString << "\n";

	//cout<<  "output: " << Form("%s_weighted.root", (fileName.substr(0, fileName.find_last_of("."))).c_str())<<endl;
        assert(inputFile);
	TTree *inputTree = (TTree*)inputFile->Get("ntuples/llp");
	...
	for (int n=0;n<inputTree->GetEntries();n++) 
	  { 
	    if (n%500==0) cout << "Processed Event " << n << "\n";
	...
	cout << "Events in the output: " << normalizedTree->GetEntries() << endl;
	//save
	outputFile->cd();
	...
	/*
        ...
        while((key = (TKey*)nextkey())){
            ...
            cout << "Getting key from file loop: " << className << endl;
	}
	cout << "\n" << endl;
