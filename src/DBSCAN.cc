...
int DBSCAN::result() {
  ...
  for (int i = 0; i < nClusters; i++) {
    float avg_x(0.0), avg_y(0.0), avg_z(0.0), avg_tWire(0.0),
        avg_tWirePruned(0.0), avg_t(0.0), avg_tTotal(0.0),
        tTotalSpreadPruned(0.0);
    ...
    for (iter = m_points.begin(); iter != m_points.end(); ++iter) {
      if (iter->clusterID == i + 1) {
        if (iter->superlayer ==
            2) // for DT rechits that only have coordinates in Z
        {
          avg_x_sl2 += iter->x;
          avg_y_sl2 += iter->y;
          avg_z_sl2 += iter->z;
    ...
    if (size_xy > 0 && size_z > 0) // for DT correct position, calculate average
                                   // Z using sl2 and average XY using sl1/3
    {
      avg_x = avg_x / size_xy;
      avg_y = avg_y / size_xy;
    ...
    do {
      ...
      timeVtx = 0;
      timeSpread = 0;
      for (std::vector<hits>::iterator it = cscHits.begin();
           it != cscHits.end(); ++it) {
      ...
      for (unsigned int i = 0; i < cscHits.size(); i++) {
        diff_tvtx = (cscHits[i].time - timeVtx) * (cscHits[i].time - timeVtx) *
                    cscHits[i].error;
    ...
    int count = 0;
    for (std::vector<hits>::iterator it = cscHits.begin(); it != cscHits.end();
         ++it) {
      if (it->strip) {
...
int DBSCAN::clusterMoments() {
  ...
  for (int i = 0; i < nClusters; i++) {
    ...
    float XSpread(0.0), YSpread(0.0), ZSpread(0.0), TSpread(0.0),
        TSpreadAll(0.0), XYSpread(0.0), RSpread(0.0), DeltaRSpread(0.0);
    ...
    for (iter = m_points.begin(); iter != m_points.end(); ++iter) {
      if (iter->clusterID == i + 1) {
        ...
        m22 += deltaPhi(iter->phi, clusterPhi[i]) *
               deltaPhi(iter->phi, clusterPhi[i]);
        DeltaRSpread +=
            pow(deltaR(clusterEta[i], clusterPhi[i], iter->eta, iter->phi), 2);
        ...
        TSpread +=
            (iter->t - clusterTimeTotal[i]) * (iter->t - clusterTimeTotal[i]);
        TSpreadAll += (iter->t - clusterTimeWeighted[i]) *
                      (iter->t - clusterTimeWeighted[i]);
        float radius = sqrt(pow(iter->x, 2) + pow(iter->y, 2));
        RSpread += pow(radius - sqrt(clusterX[i] * clusterX[i] +
                                     clusterY[i] * clusterY[i]),
                       2);
      }
    }
    ...
    float a = (m11 + m22) / 2;
    float b =
        0.5 * sqrt((m11 + m22) * (m11 + m22) - 4 * (m11 * m22 - m12 * m12));
    ...
    clusterTimeSpreadWeightedAll.push_back(
        sqrt(TSpreadAll / (float)clusterSize[i]));
  ...
  for (int i = 0; i < nClusters; i++) {
    ...
    for (unsigned int l = 0; l < m_pointSize; l++) {
      if (m_points[l].clusterID == i + 1) {
        ...
        if (m_points[l].chamber == 11)
          cscLayersPlus11.push_back(m_points[l].layer);
        if (m_points[l].chamber == 12)
          cscLayersPlus12.push_back(m_points[l].layer);
        if (m_points[l].chamber == 13)
          cscLayersPlus13.push_back(m_points[l].layer);
        if (m_points[l].chamber == 21)
          cscLayersPlus21.push_back(m_points[l].layer);
        if (m_points[l].chamber == 22)
          cscLayersPlus22.push_back(m_points[l].layer);
        if (m_points[l].chamber == 31)
          cscLayersPlus31.push_back(m_points[l].layer);
        if (m_points[l].chamber == 32)
          cscLayersPlus32.push_back(m_points[l].layer);
        if (m_points[l].chamber == 41)
          cscLayersPlus41.push_back(m_points[l].layer);
        if (m_points[l].chamber == 42)
          cscLayersPlus42.push_back(m_points[l].layer);

        if (m_points[l].chamber == -11)
          cscLayersMinus11.push_back(m_points[l].layer);
        if (m_points[l].chamber == -12)
          cscLayersMinus12.push_back(m_points[l].layer);
        if (m_points[l].chamber == -13)
          cscLayersMinus13.push_back(m_points[l].layer);
        if (m_points[l].chamber == -21)
          cscLayersMinus21.push_back(m_points[l].layer);
        if (m_points[l].chamber == -22)
          cscLayersMinus22.push_back(m_points[l].layer);
        if (m_points[l].chamber == -31)
          cscLayersMinus31.push_back(m_points[l].layer);
        if (m_points[l].chamber == -32)
          cscLayersMinus32.push_back(m_points[l].layer);
        if (m_points[l].chamber == -41)
          cscLayersMinus41.push_back(m_points[l].layer);
        if (m_points[l].chamber == -42)
          cscLayersMinus42.push_back(m_points[l].layer);

        if (abs(m_points[l].chamber) == 11)
          nSegments_Me11++;
        if (abs(m_points[l].chamber) == 12)
          nSegments_Me12++;
        if (m_points[l].chamber == 11)
          tmpCluster.nCscSegmentChamberPlus11++;
        if (m_points[l].chamber == 12)
          tmpCluster.nCscSegmentChamberPlus12++;
        if (m_points[l].chamber == 13)
          tmpCluster.nCscSegmentChamberPlus13++;
        if (m_points[l].chamber == 21)
          tmpCluster.nCscSegmentChamberPlus21++;
        if (m_points[l].chamber == 22)
          tmpCluster.nCscSegmentChamberPlus22++;
        if (m_points[l].chamber == 31)
          tmpCluster.nCscSegmentChamberPlus31++;
        if (m_points[l].chamber == 32)
          tmpCluster.nCscSegmentChamberPlus32++;
        if (m_points[l].chamber == 41)
          tmpCluster.nCscSegmentChamberPlus41++;
        if (m_points[l].chamber == 42)
          tmpCluster.nCscSegmentChamberPlus42++;
        if (m_points[l].chamber == -11)
          tmpCluster.nCscSegmentChamberMinus11++;
        if (m_points[l].chamber == -12)
          tmpCluster.nCscSegmentChamberMinus12++;
        if (m_points[l].chamber == -13)
          tmpCluster.nCscSegmentChamberMinus13++;
        if (m_points[l].chamber == -21)
          tmpCluster.nCscSegmentChamberMinus21++;
        if (m_points[l].chamber == -22)
          tmpCluster.nCscSegmentChamberMinus22++;
        if (m_points[l].chamber == -31)
          tmpCluster.nCscSegmentChamberMinus31++;
        if (m_points[l].chamber == -32)
          tmpCluster.nCscSegmentChamberMinus32++;
        if (m_points[l].chamber == -41)
          tmpCluster.nCscSegmentChamberMinus41++;
        if (m_points[l].chamber == -42)
          tmpCluster.nCscSegmentChamberMinus42++;

        if (m_points[l].station == 1)
          tmpCluster.nDtSegmentStation1++;
        if (m_points[l].station == 2)
          tmpCluster.nDtSegmentStation2++;
        if (m_points[l].station == 3)
          tmpCluster.nDtSegmentStation3++;
        if (m_points[l].station == 4)
          tmpCluster.nDtSegmentStation4++;
      }
    }
    ...
    tmpCluster.nLayersChamberPlus11 =
        std::unique(cscLayersPlus11.begin(), cscLayersPlus11.end()) -
        cscLayersPlus11.begin();
    tmpCluster.nLayersChamberPlus12 =
        std::unique(cscLayersPlus12.begin(), cscLayersPlus12.end()) -
        cscLayersPlus12.begin();
    tmpCluster.nLayersChamberPlus13 =
        std::unique(cscLayersPlus13.begin(), cscLayersPlus13.end()) -
        cscLayersPlus13.begin();
    tmpCluster.nLayersChamberPlus21 =
        std::unique(cscLayersPlus21.begin(), cscLayersPlus21.end()) -
        cscLayersPlus21.begin();
    tmpCluster.nLayersChamberPlus22 =
        std::unique(cscLayersPlus22.begin(), cscLayersPlus22.end()) -
        cscLayersPlus22.begin();
    tmpCluster.nLayersChamberPlus31 =
        std::unique(cscLayersPlus31.begin(), cscLayersPlus31.end()) -
        cscLayersPlus31.begin();
    tmpCluster.nLayersChamberPlus32 =
        std::unique(cscLayersPlus32.begin(), cscLayersPlus32.end()) -
        cscLayersPlus32.begin();
    tmpCluster.nLayersChamberPlus41 =
        std::unique(cscLayersPlus41.begin(), cscLayersPlus41.end()) -
        cscLayersPlus41.begin();
    tmpCluster.nLayersChamberPlus42 =
        std::unique(cscLayersPlus42.begin(), cscLayersPlus42.end()) -
        cscLayersPlus42.begin();
    tmpCluster.nLayersChamberMinus11 =
        std::unique(cscLayersMinus11.begin(), cscLayersMinus11.end()) -
        cscLayersMinus11.begin();
    tmpCluster.nLayersChamberMinus12 =
        std::unique(cscLayersMinus12.begin(), cscLayersMinus12.end()) -
        cscLayersMinus12.begin();
    tmpCluster.nLayersChamberMinus13 =
        std::unique(cscLayersMinus13.begin(), cscLayersMinus13.end()) -
        cscLayersMinus13.begin();
    tmpCluster.nLayersChamberMinus21 =
        std::unique(cscLayersMinus21.begin(), cscLayersMinus21.end()) -
        cscLayersMinus21.begin();
    tmpCluster.nLayersChamberMinus22 =
        std::unique(cscLayersMinus22.begin(), cscLayersMinus22.end()) -
        cscLayersMinus22.begin();
    tmpCluster.nLayersChamberMinus31 =
        std::unique(cscLayersMinus31.begin(), cscLayersMinus31.end()) -
        cscLayersMinus31.begin();
    tmpCluster.nLayersChamberMinus32 =
        std::unique(cscLayersMinus32.begin(), cscLayersMinus32.end()) -
        cscLayersMinus32.begin();
    tmpCluster.nLayersChamberMinus41 =
        std::unique(cscLayersMinus41.begin(), cscLayersMinus41.end()) -
        cscLayersMinus41.begin();
    tmpCluster.nLayersChamberMinus42 =
        std::unique(cscLayersMinus42.begin(), cscLayersMinus42.end()) -
        cscLayersMinus42.begin();
    ...
    int max_chamber =
        999; // station with the maximum number of cscsegment in this cluster
    int max_chamber_segment =
        0; // station with the maximum number of cscsegment in this cluster
    ...
    cscStations.resize(std::distance(cscStations.begin(),
                                     station_it)); // list of unique stations
    int max_station =
        999; // station with the maximum number of cscsegment in this cluster
    int max_station_segment =
        0; // station with the maximum number of cscsegment in this cluster
    ...
    tmpCluster.avgStation10perc =
        1.0 * tmpCluster.avgStation10perc / nSeg10perc;
  ...
  while (modified) {
    ...
    for (unsigned int i = 0; i < clusterEta.size(); i++) {
      for (unsigned int j = i + 1; j < clusterEta.size(); j++) {
        float current_dR =
            deltaR(clusterEta[i], clusterPhi[i], clusterEta[j], clusterPhi[j]);
        if (current_dR < mindR) {
          mindR = current_dR;
          cluster1 = i;
          cluster2 = j;
    ...
    if (mindR < MERGE_CLUSTER_DR) {
      ...
      for (iter = m_points.begin(); iter != m_points.end(); ++iter) {
        ...
        if (iter->clusterID > cluster2 + 1)
          iter->clusterID = iter->clusterID - 1;
      }
      clusterEta.erase(clusterEta.begin() + cluster2);
      clusterPhi.erase(clusterPhi.begin() + cluster2);
      nClusters--;
      modified = true;
      // can't use cscCluster, because its sorted, but the other vectors and
      // clusterID are not sorted.
    }
  }
  clear_clusters(); // clear all the vectors, but nClusters is kept to keep
                    // track of the number of clusters.
}
...
int DBSCAN::vertexing() {
  for (int i = 0; i < nClusters; i++) {
    ...
    for (unsigned int j = 0; j < m_pointSize; j++) {
      if (m_points[j].clusterID == i + 1) {
        ...
        double slope = (vecCsc.Perp() - (vecCsc + vecDir).Perp()) /
                       (vecCsc.Z() - (vecCsc + vecDir).Z());
        double beta = -1.0 * vecCsc.Z() * slope + vecCsc.Perp();
        gr->SetPoint(j, slope, beta);
      }
    }
    // Here's where the fit happens
    // Process is repeated until a good vertex is found or fewer than 3 segments
    // left The segments form a line in RZ plane Ideally, the vertex (z,r) is a
    // point on every line in the cluster So using all the measured segment
    // slopes (a_i) and a vertex guess (z,r) gives predicted intercept (b_i) r =
    // a_i*z + b_i --> b_i = r - a_i*z Fitting (a_i, b_i) points to "b_i = p[0]
    // + p[1]*a_i" by minimizing chi-squared between measured andß predicted
    // intercepts gives best (r,z) estimate Then calculate distance of closest
    // approach of all segment lines and vertex If max distance > 30cm throw out
    // farthest point and refit until all segments within 30cm or < 3 segments
    // left
    ...
    double farSegment = -1;
    int vertexN1cm(0), vertexN5cm(0), vertexN10cm(0), vertexN15cm(0),
        vertexN20cm(0), vertexN(0);
    ...
    while (!goodVtx && gr->GetN() >= 3) {
      ...
      for (int j = 0; j < gr->GetN(); j++) {
        // Distance from point (x_0, y_0) to line ax + by + c = 0 is |ax_0 +
        // by_0 + c| / sqrt(a^2 + b^2) Here: a = slope = "X"; b = -1,;c =
        // intercept = "Y"; (x_0, y_0) = (z, r) = (-p[1], p[0]);
        distance = abs(gr->GetX()[j] * fit->GetParameter(1) * -1.0 +
                       -1.0 * fit->GetParameter(0) + gr->GetY()[j]);
    ...
    for (int j = 0; j < gr->GetN(); j++) {
      // Distance from point (x_0, y_0) to line ax + by + c = 0 is |ax_0 + by_0
      // + c| / sqrt(a^2 + b^2) Here: a = slope = "X"; b = -1,;c = intercept =
      // "Y"; (x_0, y_0) = (z, r) = (-p[1], p[0]);
      distance = abs(gr->GetX()[j] * fit->GetParameter(1) * -1.0 +
                     -1.0 * fit->GetParameter(0) + gr->GetY()[j]);
      distance = distance / sqrt(pow(gr->GetX()[j], 2) + pow(-1.0, 2));
      if (distance < 1.0)
  ...
  // cout << "vertex? " << goodVtx << ", # segments = " << gr->GetN() << ",
  // maxDistance = " << maxDistance << endl; cout << "vertex R, Z, N:  " <<
  // clusterVertexR <<", " << clusterVertexZ << ",  " << clusterVertexN << endl;
...
int DBSCAN::expandCluster(Point point, int clusterID) {
  vector<int> clusterSeeds =
      calculateCluster(point); // neighbors, including itself
  ...
  } else // core points
  {
    ...
    for (iterSeeds = clusterSeeds.begin(); iterSeeds != clusterSeeds.end();
         ++iterSeeds) {
      m_points.at(*iterSeeds).clusterID =
          clusterID; // setting all the neighbors to the same clusterID
      // get the index of the core point itself

      // if (m_points.at(*iterSeeds).x == point.x && m_points.at(*iterSeeds).y
      // == point.y && m_points.at(*iterSeeds).z == point.z )
      if (m_points.at(*iterSeeds).eta == point.eta &&
          m_points.at(*iterSeeds).phi == point.phi) {
    ...
    // now clusterSeeds only contains neighbors, not itself; loop through the
    // neighbors
...
inline double DBSCAN::calculateDistance(Point pointCore, Point pointTarget) {
  // return sqrt(pow(pointCore.x - pointTarget.x,2)+pow(pointCore.y -
  // pointTarget.y,2)+pow(pointCore.z - pointTarget.z,2)); return
  // sqrt(pow(pointCore.eta - pointTarget.eta,2)+pow(deltaPhi(pointCore.phi,
  // pointTarget.phi),2)+pow((pointCore.t - pointTarget.t)/100.0,2));
  return sqrt(pow(pointCore.eta - pointTarget.eta, 2) +
              pow(deltaPhi(pointCore.phi, pointTarget.phi), 2));
}
