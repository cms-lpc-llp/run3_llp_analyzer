...
void RazorAnalyzerMerged::EnableAll() { fChain->SetBranchStatus("*", 1); }

void RazorAnalyzerMerged::Analyze(bool isData, int option,
                                  string outputFileName, string label) {
...
// I hope no one will be mad if I just add yet another meaningless copy of the
// same code :)
...
double RazorAnalyzerMerged::deltaR(double eta1, double phi1, double eta2,
                                   double phi2) {
...
TLorentzVector RazorAnalyzerMerged::makeTLorentzVector(double pt, double eta,
                                                       double phi,
                                                       double energy) {
  TLorentzVector vec;
...
TLorentzVector RazorAnalyzerMerged::makeTLorentzVectorPtEtaPhiM(double pt,
                                                                double eta,
                                                                double phi,
                                                                double mass) {
  TLorentzVector vec;
...
void RazorAnalyzer::Analyze(bool isData, int option, string outputFileName,
                            string label) {
  cout << "Analyze method called on base RazorAnalyzer instance.  Parameters "
          "were: "
       << isData << " " << option << " " << outputFileName << " " << label
       << endl;
}

// NOTE: the functions below need to be maintained by hand.  If variables are
// added or removed from the ntuple, these functions need to be updated to
// reflect the changes.
...
double RazorAnalyzer::deltaR(double eta1, double phi1, double eta2,
                             double phi2) {
  double dphi = deltaPhi(phi1, phi2);
  double deta = eta1 - eta2;
  return sqrt(dphi * dphi + deta * deta);
...
TLorentzVector RazorAnalyzer::makeTLorentzVector(double pt, double eta,
...
TLorentzVector RazorAnalyzer::makeTLorentzVectorPtEtaPhiM(double pt, double eta,
                                                          double phi,
                                                          double mass) {
  TLorentzVector vec;
  vec.SetPtEtaPhiM(pt, eta, phi, mass);
