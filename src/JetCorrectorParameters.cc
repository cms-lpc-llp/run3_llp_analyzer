...
JetCorrectorParameters::Definitions::Definitions(
    ...
    const std::vector<std::string> &fParVar, const std::string &fFormula,
    bool fIsResponse) {
...
//--- reads the member variables from a string ---------------------------
//------------------------------------------------------------------------
JetCorrectorParameters::Record::Record(const std::string &fLine, unsigned fNvar)
    : mMin(0), mMax(0) {
  mNvar = fNvar;
  ...
  if (!tokens.empty()) {
    if (tokens.size() < 3) {
      std::stringstream sserr;
      sserr << "(line " << fLine << "): " << "three tokens expected, "
            << tokens.size() << " provided.";
    ...
    if (nParam != tokens.size() - (2 * mNvar + 1)) {
      std::stringstream sserr;
      sserr << "(line " << fLine << "): " << tokens.size() - (2 * mNvar + 1)
...
//------------------------------------------------------------------------
int JetCorrectorParameters::neighbourBin(unsigned fIndex, unsigned fVar,
                                         bool fNext) const {
  int result = -1;
  unsigned N = mDefinitions.nBinVar();
  if (fVar >= N) {
    std::stringstream sserr;
    sserr << "# of bin variables " << N
...
unsigned JetCorrectorParameters::size(unsigned fVar) const {
  if (fVar >= mDefinitions.nBinVar()) {
    std::stringstream sserr;
    sserr << "requested bin variable index " << fVar
...
void JetCorrectorParameters::printScreen() const {
  ...
  std::cout << "--------------------------------------------" << std::endl;
  std::cout << "Number of binning variables:   " << definitions().nBinVar()
            << std::endl;
  ...
  std::cout << std::endl;
  std::cout << "--------------------------------------------" << std::endl;
  std::cout << "Number of parameter variables: " << definitions().nParVar()
            << std::endl;
  ...
  std::cout << std::endl;
  std::cout << "--------------------------------------------" << std::endl;
  std::cout << "Parametrization Formula:       " << definitions().formula()
            << std::endl;
  ...
  std::cout << "Correction Level:              " << definitions().level()
            << std::endl;
  std::cout << "--------------------------------------------" << std::endl;
...
void JetCorrectorParameters::printFile(const std::string &fFileName) const {
  ...
  for (unsigned i = 0; i < size(); i++) {
    for (unsigned j = 0; j < definitions().nBinVar(); j++)
      txtFile << record(i).xMin(j) << std::setw(15) << record(i).xMax(j)
              << std::setw(15);
...
const char *JetCorrectorParametersCollection::labelsArray_
    [JetCorrectorParametersCollection::N_LEVELS] = {
        "L1Offset", "L2Relative", "L3Absolute",  "L4EMF",        "L5Flavor",
        "L6UE",     "L7Parton",   "L1JPTOffset", "L2L3Residual", "Uncertainty"};

const char *JetCorrectorParametersCollection::l5FlavorArray_
    [JetCorrectorParametersCollection::N_L5_SPECIES] = {
        "L5Flavor_bJ", "L5Flavor_cJ", "L5Flavor_qJ", "L5Flavor_gJ",
        "L5Flavor_bT", "L5Flavor_cT", "L5Flavor_qT", "L5Flavor_gT"};

const char *JetCorrectorParametersCollection::l7PartonArray_
    [JetCorrectorParametersCollection::N_L7_SPECIES] = {
        "L7Parton_gJ", "L7Parton_qJ", "L7Parton_cJ",
        "L7Parton_bJ", "L7Parton_jJ", "L7Parton_qT",
        "L7Parton_cT", "L7Parton_bT", "L7Parton_tT"};

std::vector<std::string> JetCorrectorParametersCollection::labels_(
    labelsArray_, labelsArray_ + sizeof(labelsArray_) / sizeof(*labelsArray_));
...
void JetCorrectorParametersCollection::getSections(
    std::string inputFile, std::vector<std::string> &outputs) {
  outputs.clear();
...
void JetCorrectorParametersCollection::push_back(key_type i,
  ...
  if (isL5(i)) {
    ...
    std::cout
        << "***** NOT ADDING " << flav
        << ", corresponding position in JetCorrectorParameters is not found."
        << std::endl;
  }
}
...
JetCorrectorParametersCollection::findKey(std::string const &label) const {
  ...
  // throw cms::Exception("InvalidInput") << " Cannot find label " << label <<
  // std::endl;
