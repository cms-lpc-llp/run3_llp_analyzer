...
int main(int argc, char *argv[]) {
  ...
  if (argc < 4) {
    cerr << "usage MergeNtuple [inputfile1] [inputfile2] [outputfile] "
            "[analysisTag]"
         << endl;
    return -1;
  }
  ...
  cout << "filenameNTuplerList.c_str() = " << filenameNTuplerList.c_str()
       << endl;
  ...
  while (getline(inputNtuple, curNtupleName)) {
    ...
    if (NtuplesLoaded == 0) {
      ...
      std::cout << "[INFO]: loading file: " << curNtupleName.c_str()
                << std::endl;
      ...
      if (f_0->GetDirectory("ntuples")) {
        ...
        ntupleChain->SetName("llp");
        std::cout << "[INFO]: alternative configuration for tchain"
                  << std::endl;
      }
  ...
  is_mc = false;
  cout << "Loaded ntuples: " << NtuplesLoaded << " files and "
       << ntupleChain->GetEntries() << " events. is_mc = " << is_mc << endl;
  ...
  std::cout << "Loaded nanoAOD: " << NFilesLoaded << " files and "
            << nanoChain->GetEntries() << " events" << endl;
  if (nanoChain == NULL)
    return -1;
  ...
  cout << "Matched events    = " << nMatchedEvents << " / " << NEventsTree1
       << " \n";
  cout << "Un-Matched events = " << NEventsTree1 - nMatchedEvents << " / "
       << NEventsTree1 << " \n";
  //*****************************************************************************************
  // Produce Output Tree
  //*****************************************************************************************
  ...
  const char *addBranchNamesFloat[numFloatBranches]{
      "cscRechitsPhi",    "cscRechitsEta",      "cscRechitsX",
      "cscRechitsY",      "cscRechitsZ",        "cscRechitsTpeak",
      "cscRechitsTwire",  "dtRechitCorrectX",   "dtRechitCorrectY",
      "dtRechitCorrectZ", "dtRechitCorrectEta", "dtRechitCorrectPhi",
  };
  Float_t *addBranchesRazorVarFloat[numFloatBranches]{
      ntuple.cscRechitsPhi,      ntuple.cscRechitsEta,
      ntuple.cscRechitsX,        ntuple.cscRechitsY,
      ntuple.cscRechitsZ,        ntuple.cscRechitsTpeak,
      ntuple.cscRechitsTwire,    ntuple.dtRechitCorrectX,
      ntuple.dtRechitCorrectY,   ntuple.dtRechitCorrectZ,
      ntuple.dtRechitCorrectEta, ntuple.dtRechitCorrectPhi,
  };
  ...
  for (int i = 0; i < numFloatBranches; i++) {
    ...
    if (string(addBranchNamesFloat[i]).find("cscRechits") != std::string::npos)
      outputTree->Branch(
          addBranchNamesFloat[i], addBranchesInputVarFloat[i],
          TString::Format("%s[nCscRechits]/F", addBranchNamesFloat[i]));
    if (string(addBranchNamesFloat[i]).find("dtRechit") != std::string::npos)
      outputTree->Branch(
          addBranchNamesFloat[i], addBranchesInputVarFloat[i],
          TString::Format("%s[nDtRechits]/F", addBranchNamesFloat[i]));
    outputTree->SetBranchAddress(addBranchNamesFloat[i],
                                 addBranchesInputVarFloat[i]);
  }
  ...
  const char *addBranchNamesFloatSeg[numFloatSegBranches]{
      "cscSegPhi", "cscSegEta", "dtSegPhi", "dtSegEta", "rpcPhi",   "rpcEta",
      "rpcX",      "rpcY",      "rpcZ",     "rpcT",     "rpcTError"};
  Float_t *addBranchesRazorVarFloatSeg[numFloatSegBranches]{
      ntuple.cscSegPhi, ntuple.cscSegEta, ntuple.dtSegPhi, ntuple.dtSegEta,
      ntuple.rpcPhi,    ntuple.rpcEta,    ntuple.rpcX,     ntuple.rpcY,
      ntuple.rpcZ,      ntuple.rpcT,      ntuple.rpcTError};
  ...
  for (int i = 0; i < numFloatSegBranches; i++) {
    ...
    if (string(addBranchNamesFloatSeg[i]).find("cscSeg") != std::string::npos)
      outputTree->Branch(
          addBranchNamesFloatSeg[i], addBranchesInputVarFloatSeg[i],
          TString::Format("%s[nCscSeg]/F", addBranchNamesFloatSeg[i]));
    if (string(addBranchNamesFloatSeg[i]).find("dtSeg") != std::string::npos)
      outputTree->Branch(
          addBranchNamesFloatSeg[i], addBranchesInputVarFloatSeg[i],
          TString::Format("%s[nDtSeg]/F", addBranchNamesFloatSeg[i]));
    if (string(addBranchNamesFloatSeg[i]).find("rpc") != std::string::npos)
      outputTree->Branch(
          addBranchNamesFloatSeg[i], addBranchesInputVarFloatSeg[i],
          TString::Format("%s[nRpc]/F", addBranchNamesFloatSeg[i]));
    outputTree->SetBranchAddress(addBranchNamesFloatSeg[i],
                                 addBranchesInputVarFloatSeg[i]);
  }
  ////////////////////////////////////////////////
  ...
  const char *addBranchNamesInt[numIntBranches]{
      "cscRechitsChamber", "cscRechitsStation", "cscRechitsDetId",
      "dtRechitStation",   "dtRechitWheel",     "dtRechitSuperLayer"};

  Int_t *addBranchesRazorVarInt[numIntBranches]{
      ntuple.cscRechitsChamber, ntuple.cscRechitsStation,
      ntuple.cscRechitsDetId,   ntuple.dtRechitStation,
      ntuple.dtRechitWheel,     ntuple.dtRechitSuperLayer};
  ...
  for (int i = 0; i < numIntBranches; i++) {
    ...
    if (string(addBranchNamesInt[i]).find("cscRechits") != std::string::npos)
      outputTree->Branch(
          addBranchNamesInt[i], addBranchesInputVarInt[i],
          TString::Format("%s[nCscRechits]/I", addBranchNamesInt[i]));
    else if (string(addBranchNamesInt[i]).find("dtRechit") != std::string::npos)
      outputTree->Branch(
          addBranchNamesInt[i], addBranchesInputVarInt[i],
          TString::Format("%s[nDtRechits]/I", addBranchNamesInt[i]));
    outputTree->SetBranchAddress(addBranchNamesInt[i],
                                 addBranchesInputVarInt[i]);
  }
  ...
  const char *addBranchNamesIntSeg[numIntSegBranches]{
      "cscSegChamber", "cscSegStation", "cscSegNRecHits", "dtSegStation",
      "dtSegWheel",    "rpcBx",         "rpcRegion",      "rpcRing",
      "rpcSector",     "rpcStation",    "rpcLayer"};

  Int_t *addBranchesRazorVarIntSeg[numIntSegBranches]{
      ntuple.cscSegChamber, ntuple.cscSegStation, ntuple.cscSegNRecHits,
      ntuple.dtSegStation,  ntuple.dtSegWheel,    ntuple.rpcBx,
      ntuple.rpcRegion,     ntuple.rpcRing,       ntuple.rpcSector,
      ntuple.rpcStation,    ntuple.rpcLayer
  ...
  for (int i = 0; i < numIntSegBranches; i++) {
    ...
    if (string(addBranchNamesIntSeg[i]).find("cscSeg") != std::string::npos)
      outputTree->Branch(
          addBranchNamesIntSeg[i], addBranchesInputVarIntSeg[i],
          TString::Format("%s[nCscSeg]/I", addBranchNamesIntSeg[i]));
    else if (string(addBranchNamesIntSeg[i]).find("dtSeg") != std::string::npos)
      outputTree->Branch(
          addBranchNamesIntSeg[i], addBranchesInputVarIntSeg[i],
          TString::Format("%s[nDtSeg]/I", addBranchNamesIntSeg[i]));
    else if (string(addBranchNamesIntSeg[i]).find("rpc") != std::string::npos)
      outputTree->Branch(
          addBranchNamesIntSeg[i], addBranchesInputVarIntSeg[i],
          TString::Format("%s[nRpc]/I", addBranchNamesIntSeg[i]));
    outputTree->SetBranchAddress(addBranchNamesIntSeg[i],
                                 addBranchesInputVarIntSeg[i]);
  }

  int nGLLP;
  float gLLP_eta[2], gLLP_phi[2], gLLP_csc[2], gLLP_dt[2], gLLP_beta[2],
      gLLP_e[2], gLLP_pt[2], gLLP_decay_vertex_r[2], gLLP_decay_vertex_x[2],
      gLLP_decay_vertex_y[2], gLLP_decay_vertex_z[2];
  if (is_mc) {
    ...
    outputTree->Branch("gLLP_decay_vertex_r", gLLP_decay_vertex_r,
                       "gLLP_decay_vertex_r[nGLLP]/F");
    outputTree->Branch("gLLP_decay_vertex_x", gLLP_decay_vertex_x,
                       "gLLP_decay_vertex_x[nGLLP]/F");
    outputTree->Branch("gLLP_decay_vertex_y", gLLP_decay_vertex_y,
                       "gLLP_decay_vertex_y[nGLLP]/F");
    outputTree->Branch("gLLP_decay_vertex_z", gLLP_decay_vertex_z,
  ...
  for (uint n = 0; n < NEventsTree1; n++) {
    ...
    // if (ntuple.ncscRechits >= 20000) cout<< n << ", " << ntuple.ncscRechits<<
    // endl; if (ntuple.nCscSeg >= 300) cout<< n << ", " << ntuple.nCscSeg<<
    // endl;
    ...
    int nCscRechitsChamberPlus11 = 0, nCscRechitsChamberPlus12 = 0,
        nCscRechitsChamberPlus13 = 0, nCscRechitsChamberPlus21 = 0,
        nCscRechitsChamberPlus22 = 0, nCscRechitsChamberPlus31 = 0,
        nCscRechitsChamberPlus32 = 0, nCscRechitsChamberPlus41 = 0,
        nCscRechitsChamberPlus42 = 0, nCscRechitsChamberMinus11 = 0,
        nCscRechitsChamberMinus12 = 0, nCscRechitsChamberMinus13 = 0,
        nCscRechitsChamberMinus21 = 0, nCscRechitsChamberMinus22 = 0,
        nCscRechitsChamberMinus31, nCscRechitsChamberMinus32 = 0,
        nCscRechitsChamberMinus41 = 0, nCscRechitsChamberMinus42 = 0,
        nCscRings = 0;
    ...
    if (!is_mc && nDtRings + nCscRings >= 10)
      continue;
    ...
    bool noise_filter =
        nano.Flag_eeBadScFilter && nano.Flag_hfNoisyHitsFilter &&
        nano.Flag_BadPFMuonDzFilter && nano.Flag_BadPFMuonFilter &&
        nano.Flag_EcalDeadCellTriggerPrimitiveFilter &&
        nano.Flag_globalSuperTightHalo2016Filter && nano.Flag_goodVertices;
    if (!is_mc && !noise_filter)
      continue;

    // if (ntuple.ncscRechits > 8000)
    //{
    //  std::cerr << "number of csc rechits is too high for " << ntuple.runNum
    //  << ":" << ntuple.eventNum << std::endl; std::cerr << "(nCscRechits,
    //  nDtRechits) = (" << ntuple.ncscRechits << ", " << ntuple.nDtRechits <<
    //  ")" << std::endl; throw std::overflow_error("number of csc rechits is
    //  too high");
    //}
    // if (ntuple.nDtRechits > 8000)
    //{
    //  std::cerr << "number of dt rechits is too high for " << ntuple.runNum <<
    //  ":" << ntuple.eventNum << std::endl; std::cerr << "(nCscRechits,
    //  nDtRechits) = (" << ntuple.ncscRechits << ", " << ntuple.nDtRechits <<
    //  ")" << std::endl; throw std::overflow_error("number of dt rechits is too
    //  high");
    //}

    for (int i = 0; i < numFloatBranches; i++) {
      int temp_nhits = 0;
      if (string(addBranchNamesFloat[i]).find("cscRechits") !=
          std::string::npos)
