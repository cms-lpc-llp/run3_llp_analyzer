...
int CACluster::run() {
  ...
  for (auto const &fjJet : fjJets) {
    ...
    if (int(fjJet.constituents().size()) < m_minPoints)
      continue;
...
void CACluster::clusterProperties() {
  for (int i = 0; i < nClusters; i++) {
    ...
    avg_eta = atan(sqrt(pow(tmpCluster.x, 2) + pow(tmpCluster.y, 2)) /
                   abs(tmpCluster.z));
    ...
    float m11(0.0), m12(0.0), m22(0.0), p3_x(0.0), p4_x(0.0), p3_y(0.0),
        p4_y(0.0), p3_z(0.0), p4_z(0.0);
    float XSpread(0.0), YSpread(0.0), ZSpread(0.0), TSpread(0.0),
        TSpreadAll(0.0), XYSpread(0.0), RSpread(0.0), DeltaRSpread(0.0);
    int nXY = 0;
    int nZ = 0;

    tmpCluster.nCscRechitsChamberPlus11 = 99999;
    tmpCluster.nCscRechitsChamberPlus12 = 99999;
    tmpCluster.nCscRechitsChamberPlus13 = 99999;
    tmpCluster.nCscRechitsChamberPlus21 = 99999;
    tmpCluster.nCscRechitsChamberPlus22 = 99999;
    tmpCluster.nCscRechitsChamberPlus31 = 99999;
    tmpCluster.nCscRechitsChamberPlus32 = 99999;
    tmpCluster.nCscRechitsChamberPlus41 = 99999;
    tmpCluster.nCscRechitsChamberPlus42 = 99999;

    tmpCluster.nCscRechitsChamberMinus11 = 99999;
    tmpCluster.nCscRechitsChamberMinus12 = 99999;
    tmpCluster.nCscRechitsChamberMinus13 = 99999;
    tmpCluster.nCscRechitsChamberMinus21 = 99999;
    tmpCluster.nCscRechitsChamberMinus22 = 99999;
    tmpCluster.nCscRechitsChamberMinus31 = 99999;
    tmpCluster.nCscRechitsChamberMinus32 = 99999;
    tmpCluster.nCscRechitsChamberMinus41 = 99999;
    tmpCluster.nCscRechitsChamberMinus42 = 99999;

    tmpCluster.nDtRechitsStation1 = 99999;
    tmpCluster.nDtRechitsStation2 = 99999;
    tmpCluster.nDtRechitsStation3 = 99999;
    tmpCluster.nDtRechitsStation4 = 99999;
    tmpCluster.nDtRechitsWheel0 = 99999;
    tmpCluster.nDtRechitsWheel1 = 99999;
    tmpCluster.nDtRechitsWheel2 = 99999;
    ...
    for (auto const &rechit : rechits) {
      ...
      m12 +=
          (rechit.eta - tmpCluster.eta) * deltaPhi(rechit.phi, tmpCluster.phi);
      m22 += deltaPhi(rechit.phi, tmpCluster.phi) *
             deltaPhi(rechit.phi, tmpCluster.phi);
      DeltaRSpread += pow(
          deltaR(tmpCluster.eta, tmpCluster.phi, rechit.eta, rechit.phi), 2);
      ...
      if (rechit.superlayer > 0) { // hit in DT
        ...
        if (rechit.superlayer == 1 || rechit.superlayer == 3) { // XY
                                                                // information
          ...
          RSpread += pow(radius - sqrt(tmpCluster.x * tmpCluster.x +
                                       tmpCluster.y * tmpCluster.y),
                         2);
          p3_x += (rechit.x - tmpCluster.x) * (rechit.x - tmpCluster.x) *
                  (rechit.x - tmpCluster.x);
          p4_x += (rechit.x - tmpCluster.x) * (rechit.x - tmpCluster.x) *
                  (rechit.x - tmpCluster.x) * (rechit.x - tmpCluster.x);
          p3_y += (rechit.y - tmpCluster.y) * (rechit.y - tmpCluster.y) *
                  (rechit.y - tmpCluster.y);
          p4_y += (rechit.y - tmpCluster.y) * (rechit.y - tmpCluster.y) *
                  (rechit.y - tmpCluster.y) * (rechit.y - tmpCluster.y);
          nXY++;
        } else if (rechit.superlayer == 2) { // Z information
          ZSpread += (rechit.z - tmpCluster.z) * (rechit.z - tmpCluster.z);
          p3_z += (rechit.z - tmpCluster.z) * (rechit.z - tmpCluster.z) *
                  (rechit.z - tmpCluster.z);
          p4_z += (rechit.z - tmpCluster.z) * (rechit.z - tmpCluster.z) *
                  (rechit.z - tmpCluster.z) * (rechit.z - tmpCluster.z);
          nZ++;
        }
      } else { // hit in CSC
        XYSpread += (rechit.x - tmpCluster.x) * (rechit.y - tmpCluster.y);
      ...
      TSpread +=
          (rechit.t - tmpCluster.tTotal) * (rechit.t - tmpCluster.tTotal);
      TSpreadAll +=
          (rechit.t - tmpCluster.tWeighted) * (rechit.t - tmpCluster.tWeighted);

      // number of rechits in each chamber/station
      if (rechit.chamber == 11)
        tmpCluster.nCscRechitsChamberPlus11++;
      if (rechit.chamber == 12)
        tmpCluster.nCscRechitsChamberPlus12++;
      if (rechit.chamber == 13)
        tmpCluster.nCscRechitsChamberPlus13++;
      if (rechit.chamber == 21)
        tmpCluster.nCscRechitsChamberPlus21++;
      if (rechit.chamber == 22)
        tmpCluster.nCscRechitsChamberPlus22++;
      if (rechit.chamber == 31)
        tmpCluster.nCscRechitsChamberPlus31++;
      if (rechit.chamber == 32)
        tmpCluster.nCscRechitsChamberPlus32++;
      if (rechit.chamber == 41)
        tmpCluster.nCscRechitsChamberPlus41++;
      if (rechit.chamber == 42)
        tmpCluster.nCscRechitsChamberPlus42++;
      if (rechit.chamber == -11)
        tmpCluster.nCscRechitsChamberMinus11++;
      if (rechit.chamber == -12)
        tmpCluster.nCscRechitsChamberMinus12++;
      if (rechit.chamber == -13)
        tmpCluster.nCscRechitsChamberMinus13++;
      if (rechit.chamber == -21)
        tmpCluster.nCscRechitsChamberMinus21++;
      if (rechit.chamber == -22)
        tmpCluster.nCscRechitsChamberMinus22++;
      if (rechit.chamber == -31)
        tmpCluster.nCscRechitsChamberMinus31++;
      if (rechit.chamber == -32)
        tmpCluster.nCscRechitsChamberMinus32++;
      if (rechit.chamber == -41)
        tmpCluster.nCscRechitsChamberMinus41++;
      if (rechit.chamber == -42)
        tmpCluster.nCscRechitsChamberMinus42++;

      if (abs(rechit.station) == 1)
        tmpCluster.nDtRechitsStation1++;
      if (abs(rechit.station) == 2)
        tmpCluster.nDtRechitsStation2++;
      if (abs(rechit.station) == 3)
        tmpCluster.nDtRechitsStation3++;
      if (abs(rechit.station) == 4)
        tmpCluster.nDtRechitsStation4++;
    }
    ...
    tmpCluster.KurtX =
        (p4_x / (float)nXY) / (pow(XSpread / (float)nXY, 2)) - 3.0;
    tmpCluster.KurtY =
        (p4_y / (float)nXY) / (pow(YSpread / (float)nXY, 2)) - 3.0;
...
void CACluster::sort_clusters() // only run sort after merg_clusters, or it will
                                // mess up the clusterID of the m_points
{
...
void CACluster::merge_clusters() {
  ...
  while (modified) {
    ...
    for (unsigned int i = 0; i < nClusters;
         i++) { // find the min_deltaR between any two clusters
      for (unsigned int j = i + 1; j < nClusters; j++) {
        float current_dR = deltaR(clusters[i].eta, clusters[i].phi,
                                  clusters[j].eta, clusters[j].phi);
        if (current_dR < mindR) {
          mindR = current_dR;
          cluster1 = i;
          cluster2 = j;
    ...
    if (mindR < CA_MERGE_CLUSTER_DR) { // if min deltaR < the deltaR merging
                                       // threshold, then merge the two clusters
      vector<Rechits>::iterator iter;
      ...
      for (iter = m_points.begin(); iter != m_points.end(); ++iter) {
        if (iter->clusterID ==
            cluster2) { // change the clusterID from cluster 2 to cluster1
          iter->clusterID = cluster1;
        }
        if (iter->clusterID > cluster2)
          iter->clusterID =
              iter->clusterID -
              1; // change the cluster ID for all clusters beyond to ID-1

        if (iter->clusterID ==
            cluster1) // recalculate the eta/phi position of the new cluster
        {
        ...
        if (size_xy > 0 &&
            size_z > 0) // for DT correct position, calculate average Z using
                        // sl2 and average XY using sl1/3
        {
          avg_x = avg_x / size_xy;
          avg_y = avg_y / size_xy;
          avg_z = avg_z_sl2 / size_z;
