...
public:
  ...
  public:
    ...
    Definitions(const std::vector<std::string> &fBinVar,
                ...
                const std::string &fFormula, bool fIsResponse);
  ...
  public:
    ...
    Record(unsigned fNvar, const std::vector<float> &fXMin,
    ...
    float xMiddle(unsigned fVar) const {
      return 0.5 * (xMin(fVar) + xMax(fVar));
    }
  ...
  JetCorrectorParameters(const std::string &fFile,
                         const std::string &fSection = "");
  JetCorrectorParameters(
      const JetCorrectorParameters::Definitions &fDefinitions,
