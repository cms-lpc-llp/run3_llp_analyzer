...
def return_clusterSize_dEtaClusterTau_allSelectionsDNN(events, passID = 'tauIsVLoose', failID = None, DNN_cut=0.995, blind=True, hotspotCheck=False, additional_branches = []):
    '''
    Helper function for returning (computed) cluster size branch and dPhi(cluster, tau) branches with necessary preselections
    Cannot run unblinded without inverted TauID
    Baseline Preselection Applied:
    ...
    & ((events.cscRechitClusterNRechitChamberMinus11 + events.cscRechitClusterNRechitChamberMinus12 + 
        ...
        #(events.cscRechitClusterNStation10>1) &
        (events.cscRechitClusterDNN_bkgMC_plusBeamHalo>=DNN_cut) &
        (abs(events.cscRechitClusterPuppiMet_dPhi)<1.5) & (abs(events.cscRechitClusterPromptTauDeltaPhi)>2))
    
    if blind:
        cluster_cuts = (cluster_cuts) & ~((events.cscRechitClusterSize>160) & (abs(events.cscRechitClusterPromptTauDeltaEta)<2))
    ...
    if not hotspotCheck:
        clusterSize = events.cscRechitClusterSize[cluster_cuts]
        dEta = abs(events.cscRechitClusterPromptTauDeltaEta[cluster_cuts])
        #ak.flatten(clusterSize[ak.num(clusterSize)>0]).visualize()
        return_tuple = ak.flatten(clusterSize[ak.num(clusterSize)>0]).compute(), ak.flatten(dEta[ak.num(dEta)>0]).compute() #remove empty entries
...
def return_clusterSize_dEtaClusterTau_allSelectionsDNN_HTnoMET(events, passID = 'tauIsVLoose', failID = None, DNN_cut=0.995, blind=True, hotspotCheck=False, additional_branches = []):
    '''
    Helper function for returning (computed) cluster size branch and dPhi(cluster, tau) branches with necessary preselections
    Cannot run unblinded without inverted TauID
    Baseline Preselection Applied:
    ...
    nClusters_cut = events.nCscRechitClusters>0
    #MET_cut = events.Puppimet>30
    HT_cut = (events.HTSoftJetsLargerThan2p65-events.HTSoftJets2p65to3p139)==0
    lep_cut = events.nLeptons==0

    
    events = events[(noise_filters_cut) & (trigger_cut) & (nTaus_cut) & (nClusters_cut) & (HT_cut) & (lep_cut)]
