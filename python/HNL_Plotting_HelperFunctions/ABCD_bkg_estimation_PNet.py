...
def run_ABCD_from_cutflow(events, cluster_mask, sizeCut, PNetCut, normalization_factor=1, blind=True, flavor="Tau"):
    ...
    event_counts_unc = []
    if blind:
        bin_names = ["bin A (low NHits, low PNetvsE)", "bin B (low NHits, high PNetvsE)", "bin C (high NHits, low PNetvsE)", "bin D expected (high NHits, high PNetvsE)"]
    else:
