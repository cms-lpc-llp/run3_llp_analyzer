...
def loadTree_nanoFactory(f, isMC=True, trigger="tau"):
    events = NanoEventsFactory.from_root(f,schemaclass=BaseSchema, metadata={"dataset":"HNL" }).events()
    if not isMC: 
    ...
    events = ak.with_field(events, DNN_branch, "cscRechitClusterDNN_bkgMC_plusBeamHalo")
    return events
