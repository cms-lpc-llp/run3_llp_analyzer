...
def makeCutflow(events, cfg_file, isMC=False, noGenCuts=True, Run=3, sample_ctau=1000, reweight_ctau = 1000, ABCD=True, sizeCut=160, PNetCut=0.974, dEtaCut=2,blind=True, flavor="Tau", signal_xsec=1, genEvents=1, dEta=False, return_branches=False, return_events=False, isBkg=False):
    ...
    if ABCD:
        ...
        if not dEta:
            df_ABCD = ABCD_bkg_estimation_PNet.run_ABCD_from_cutflow(cumulative_events, collection_masks['cscCluster'], sizeCut, PNetCut, normalization_factor, blind=blind, flavor=flavor, isMC=isMC)
        else:
            df_ABCD = ABCD_bkg_estimation_PNet.run_ABCD_from_cutflow_dEta(cumulative_events, collection_masks['cscCluster'], sizeCut, dEtaCut, normalization_factor, blind=blind, flavor=flavor, isMC=isMC)
        if blind:
