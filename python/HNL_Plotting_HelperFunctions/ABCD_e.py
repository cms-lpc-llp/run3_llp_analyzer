...
def return_clusterSize_dPhiClustere(events, passID = 'lepLooseId', failID = None, blind=True, hotspotCheck=False):
    '''
    ...
    #event level cuts
    noise_filters_cut = (events.Flag_all) & (events.Flag_ecalBadCalibFilter) & (events.jetVeto)
    ...
    lep_cut = events.nLeptons==0

    events = events[(noise_filters_cut) & (trigger_cut) & (nTaus_cut) & (nClusters_cut) & (MET_cut) & (lep_cut)]
    ...
    & ((events.cscRechitClusterNRechitChamberMinus11 + events.cscRechitClusterNRechitChamberMinus12 + 
        ...
        & (events.cscRechitClusterMuonVetoPt<30) & (events.cscRechitClusterJetVetoPt<10))
...
def return_clusterSize_dPhiClusterE_allSelections(events, passID = 'lepLooseId', failID = None, blind=True, hotspotCheck=False):
    '''
    Helper function for returning (computed) cluster size branch and dPhi(cluster, tau) branches with necessary preselections
    Cannot run unblinded without inverted TauID
    Baseline Preselection Applied:
    ...
    #event level cuts
    noise_filters_cut = (events.Flag_all) & (events.Flag_ecalBadCalibFilter) & (events.jetVeto)
    trigger_cut = events.HLT_CscCluster100_Ele5
    
    nClusters_cut = events.nCscRechitClusters>0
    MET_cut = events.Puppimet>30
    lep_cut = events.nLeptons==1

    events = events[(noise_filters_cut) & (trigger_cut) & (nClusters_cut) & (MET_cut) & (lep_cut)]
    ...
    if passID is not None and failID is not None:
        lep_ID_cut = ak.all(((events[passID]) & ~(events[failID])), axis=1)
    elif passID is not None:
        lep_ID_cut = ak.all(((events[passID])), axis=1)
    elif failID is not None:
        lep_ID_cut = ak.all((~(events[failID])), axis=1)
    else:
        lep_ID_cut = ak.ones_like((events.evtNum), axis=1, dtype=bool)

    lep_pdg_cut = ak.all((abs(events.lepPdgId)==11), axis=1)
    lep_pt_cut = ak.all((events.lepPt>=15), axis=1)

    events = events[(lep_ID_cut ) & (lep_pdg_cut) & (lep_pt_cut)]
    cluster_cuts = ((events.cscRechitClusterSize>=100) & (events.cscRechitClusterTimeWeighted > -5) & (events.cscRechitClusterTimeWeighted < 12.5) 
    & ((events.cscRechitClusterNRechitStation12)==0)  &
        (events.cscRechitClusterMuonVetoPt<30) & (events.cscRechitClusterJetVetoPt<10) & 
        (events.cscRechitClusterNStation10>1) &
        #(events.cscRechitClusterDNN_bkgMC_plusBeamHalo>=0.995) &
        (abs(events.cscRechitClusterPuppiMet_dPhi)<1.5) & (abs(events.cscRechitClusterPromptEleDeltaEta)<2))
    
    if blind:
        cluster_cuts = (cluster_cuts) & ~((events.cscRechitClusterSize>160) & (abs(events.cscRechitClusterPromptEleDeltaPhi)>2))
    ...
    if not hotspotCheck:
        clusterSize = events.cscRechitClusterSize[cluster_cuts]
        dPhi = abs(events.cscRechitClusterPromptEleDeltaPhi[cluster_cuts])
        return_tuple = ak.flatten(clusterSize[ak.num(clusterSize)>0]).compute(), ak.flatten(dPhi[ak.num(dPhi)>0]).compute() #remove empty entries
...
def return_clusterSize_dPhiClusterE_allSelectionsDNN(events, passID = 'lepLooseId', failID = None, DNN_cut=0.99975, blind=True, hotspotCheck=False, additional_branches = []):
    ...
    if not hotspotCheck:
        clusterSize = events.cscRechitClusterSize[cluster_cuts]
        dPhi = abs(events.cscRechitClusterPromptEleDeltaPhi[cluster_cuts])
        #ak.flatten(clusterSize[ak.num(clusterSize)>0]).visualize()
        return_tuple = ak.flatten(clusterSize[ak.num(clusterSize)>0]).compute(), ak.flatten(dPhi[ak.num(dPhi)>0]).compute() #remove empty entries
