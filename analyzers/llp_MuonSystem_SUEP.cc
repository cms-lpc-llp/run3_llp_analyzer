...
void llp_MuonSystem_SUEP::Analyze(bool isData, int options, string outputfilename, string analysisTag, int mh, int mx, int ctau, int T)
{
  ...
  TreeMuonSystemSUEP *MuonSystem = new TreeMuonSystemSUEP;
  ...
  for (Long64_t jentry=0; jentry<fChain->GetEntries();jentry++) {
    ...
    {
      MuonSystem->weight = genWeight*weight;
      NEvents->Fill(1, genWeight*weight);
    }
...

    {
      ...
      for(int i = 0; i < nGenParticle; i++)
       {
	 if(gParticleId[i]==25 && gParticleStatus[i]==62)
	   {
      ...
      //cout << "LB DEB: nGenLLP is " << nGenLLP << endl;
      //cout << "N_MAX_LLP: " << N_MAX_LLP << endl;
      ////cout << "nGLLP: " << nGLLP << endl;
      //cout << "MuonSystem->nGLLP: " << MuonSystem->nGLLP << endl;

      //for(int i = 0; i < nGenLLP; i++)
      //{
      //cout << "LB DEB: Gen LLP[" << i << "] eta: " << gLLP_eta[i] << " phi: " << gLLP_phi[i] << endl;
      //}
    ...
    for(int i = 0; i < nGenLLPDau; i++)
      {
	MuonSystem->gLLP_daughter_eta[MuonSystem->nGLLPDau] = gLLP_daughter_eta[i];
	MuonSystem->gLLP_daughter_phi[MuonSystem->nGLLPDau] = gLLP_daughter_phi[i];
	MuonSystem->gLLP_daughter_e[MuonSystem->nGLLPDau] = gLLP_daughter_e[i];
	MuonSystem->gLLP_daughter_pt[MuonSystem->nGLLPDau] = gLLP_daughter_pt[i];
	MuonSystem->gLLP_daughter_mass[MuonSystem->nGLLPDau] = gLLP_daughter_mass[i];
	MuonSystem->gLLP_daughter_id[MuonSystem->nGLLPDau] = gLLP_daughter_id[i];
	MuonSystem->gLLP_daughter_prod_vertex_x[MuonSystem->nGLLPDau] = gLLP_daughter_prod_vertex_x[i];
	MuonSystem->gLLP_daughter_prod_vertex_y[MuonSystem->nGLLPDau] = gLLP_daughter_prod_vertex_y[i];
	MuonSystem->gLLP_daughter_prod_vertex_z[MuonSystem->nGLLPDau] = gLLP_daughter_prod_vertex_z[i];
	MuonSystem->gLLP_daughter_decay_vertex_x[MuonSystem->nGLLPDau] = gLLP_daughter_decay_vertex_x[i];
	MuonSystem->gLLP_daughter_decay_vertex_y[MuonSystem->nGLLPDau] = gLLP_daughter_decay_vertex_y[i];
	MuonSystem->gLLP_daughter_decay_vertex_z[MuonSystem->nGLLPDau] = gLLP_daughter_decay_vertex_z[i];
	MuonSystem->nGLLPDau++;
      }

    //cout << "LB DEB: post nGLLP: " << MuonSystem->nGLLP << endl; 
      //get NPU
      MuonSystem->npv = nPV;
      ...
      for( int i = 0; i < nMuons; i++ )
      {
        ...
        {
          if (RazorAnalyzerSUEP::deltaR(muonEta[i],muonPhi[i],lep.lepton.Eta(),lep.lepton.Phi()) < 0.3) overlap = true;
        }
        if(overlap) continue;
      ...
      for( int i = 0; i < nElectrons; i++ )
      {
        ...
        for(auto& lep : Leptons)
        {
          if (RazorAnalyzerSUEP::deltaR(eleEta[i],elePhi[i],lep.lepton.Eta(),lep.lepton.Phi()) < 0.3) overlap = true;
        }
        if(overlap) continue;
        leptons tmpElectron;
    ...
    //cout << "nJets from ntuples: " << nJets << endl;
    for(int i = 0; i < nJets; i++)
    {
      ...
      double deltaR = -1;
      for(auto& lep : Leptons){
        double thisDR = RazorAnalyzerSUEP::deltaR(jetEta[i],jetPhi[i],lep.lepton.Eta(),lep.lepton.Phi());
      ...
      //cout << "nJets after selections: " << MuonSystem->nJets << endl;
      ...
      for (int i = 0; i < nDtRechits; i++) {
        ...
        if (dtRechitsStation[i] == 1 && dtRechitsWheel[i] == -2) nDTRechitsChamberMinus12++;
        if (dtRechitsStation[i] == 1 && dtRechitsWheel[i] == -1) nDTRechitsChamberMinus11++;
        if (dtRechitsStation[i] == 1 && dtRechitsWheel[i] == 0) nDTRechitsChamber10++;
        if (dtRechitsStation[i] == 1 && dtRechitsWheel[i] == 1) nDTRechitsChamberPlus11++;
        if (dtRechitsStation[i] == 1 && dtRechitsWheel[i] == 2) nDTRechitsChamberPlus12++;
        if (dtRechitsStation[i] == 2 && dtRechitsWheel[i] == -2) nDTRechitsChamberMinus22++;
        if (dtRechitsStation[i] == 2 && dtRechitsWheel[i] == -1) nDTRechitsChamberMinus21++;
        if (dtRechitsStation[i] == 2 && dtRechitsWheel[i] == 0) nDTRechitsChamber20++;
        if (dtRechitsStation[i] == 2 && dtRechitsWheel[i] == 1) nDTRechitsChamberPlus21++;
        if (dtRechitsStation[i] == 2 && dtRechitsWheel[i] == 2) nDTRechitsChamberPlus22++;
        if (dtRechitsStation[i] == 3 && dtRechitsWheel[i] == -2) nDTRechitsChamberMinus32++;
        if (dtRechitsStation[i] == 3 && dtRechitsWheel[i] == -1) nDTRechitsChamberMinus31++;
        if (dtRechitsStation[i] == 3 && dtRechitsWheel[i] == 0) nDTRechitsChamber30++;
        if (dtRechitsStation[i] == 3 && dtRechitsWheel[i] == 1) nDTRechitsChamberPlus31++;
        if (dtRechitsStation[i] == 3 && dtRechitsWheel[i] == 2) nDTRechitsChamberPlus32++;
        if (dtRechitsStation[i] == 4 && dtRechitsWheel[i] == -2) nDTRechitsChamberMinus42++;
        if (dtRechitsStation[i] == 4 && dtRechitsWheel[i] == -1) nDTRechitsChamberMinus41++;
        if (dtRechitsStation[i] == 4 && dtRechitsWheel[i] == 0) nDTRechitsChamber40++;
        if (dtRechitsStation[i] == 4 && dtRechitsWheel[i] == 1) nDTRechitsChamberPlus41++;
        if (dtRechitsStation[i] == 4 && dtRechitsWheel[i] == 2) nDTRechitsChamberPlus42++;
      ...
      for ( auto &tmp : ds.CscCluster ) {
          ...
          {
            if (fabs(jetEta[i]>3.0)) continue;
            if (RazorAnalyzerSUEP::deltaR(jetEta[i], jetPhi[i], MuonSystem->cscRechitClusterEta[MuonSystem->nCscRechitClusters],MuonSystem->cscRechitClusterPhi[MuonSystem->nCscRechitClusters]) < 0.4 && jetPt[i] > MuonSystem->cscRechitClusterJetVetoPt[MuonSystem->nCscRechitClusters] ) {
          ...
          for(int i = 0; i < nMuons; i++)
          {
            ...
            float muonIso = (muon_chargedIso[i] + fmax(0.0,  muon_photonIso[i] + muon_neutralHadIso[i] - 0.5*muon_pileupIso[i])) / muonPt[i];
            if (RazorAnalyzerSUEP::deltaR(muonEta[i], muonPhi[i], MuonSystem->cscRechitClusterEta[MuonSystem->nCscRechitClusters],MuonSystem->cscRechitClusterPhi[MuonSystem->nCscRechitClusters]) < 0.4 && muonPt[i] > MuonSystem->cscRechitClusterMuonVetoPt[MuonSystem->nCscRechitClusters] ) {
	      ...
	      //cout << "LB DEB: gen level matching CSC ... " << endl;
              double current_delta_r = RazorAnalyzerSUEP::deltaR(MuonSystem->cscRechitClusterEta[MuonSystem->nCscRechitClusters], MuonSystem->cscRechitClusterPhi[MuonSystem->nCscRechitClusters], MuonSystem->gLLP_eta[j], MuonSystem->gLLP_phi[j]);
              if (current_delta_r < min_deltaR)
              {
                min_deltaR = current_delta_r;
                index = j;
              }
	      //cout << "LB DEB: gen level matching CSC done! " << endl;
	    ...
	    //cout << "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~" << endl;
	    //cout << "CSC Cluster n. " << MuonSystem->nCscRechitClusters << endl;
	    //cout << "Need to pair the decay vertex of the LLP with the prod vertex of the daughter! " << endl;

	    //cout << "Matched LLP decay vertex: (x,y,z) = (" << MuonSystem->gLLP_decay_vertex_x[index] << "," << MuonSystem->gLLP_decay_vertex_y[index] << "," << MuonSystem->gLLP_decay_vertex_z[index] << ")" << endl;

	    int d1=9999999;
	    int d2=-999999;
	    for(int d = 0; d<MuonSystem->nGLLPDau; d++)
	      {
		if(MuonSystem->gLLP_daughter_prod_vertex_x[d] == MuonSystem->gLLP_decay_vertex_x[index] && MuonSystem->gLLP_daughter_prod_vertex_z[d] == MuonSystem->gLLP_decay_vertex_z[index])
		  {
		    //cout << "gdau [" << d << "] prod vertex: (" << MuonSystem->gLLP_daughter_prod_vertex_x[d]  << "," << MuonSystem->gLLP_daughter_prod_vertex_y[d] <<  "," << MuonSystem->gLLP_daughter_prod_vertex_z[d] << ")" << endl;
		    d1=min(d,d1);
		    d2=max(d,d2);
		  }
	      }
	    //cout << "Dau idx CSC: " << d1 << " " << MuonSystem->gLLP_daughter_e[d1] << endl;
	    //cout << "Dau idx CSC: " << d2 << " " << MuonSystem->gLLP_daughter_e[d2] << endl;	    
	    //cout << "LB DEB: chosen particle index CSC? " << index << endl;
            ...
            MuonSystem->cscRechitCluster_match_gLLP_dau1_e[MuonSystem->nCscRechitClusters] = MuonSystem->gLLP_daughter_e[d1];
            MuonSystem->cscRechitCluster_match_gLLP_dau2_e[MuonSystem->nCscRechitClusters] = MuonSystem->gLLP_daughter_e[d2];

	    //cout << "LB DEB: CSC-llp matching done " << index << endl;

          }

          //match to MB1 DT segments
          for (int i = 0; i < nDtSeg; i++) {
            if (RazorAnalyzerSUEP::deltaR(dtSegEta[i], dtSegPhi[i], MuonSystem->cscRechitClusterEta[MuonSystem->nCscRechitClusters],MuonSystem->cscRechitClusterPhi[MuonSystem->nCscRechitClusters]) < 0.4 )
            {
          ...
          for (int i = 0; i < nRpc; i++) {
            ...
            if (RazorAnalyzerSUEP::deltaR(rpcEta[i], rpcPhi[i], MuonSystem->cscRechitClusterEta[MuonSystem->nCscRechitClusters],MuonSystem->cscRechitClusterPhi[MuonSystem->nCscRechitClusters]) < 0.4 )
            {
          ...
          MuonSystem->cscRechitClusterMet_dPhi[MuonSystem->nCscRechitClusters] =  RazorAnalyzerSUEP::deltaPhi(MuonSystem->cscRechitClusterPhi[MuonSystem->nCscRechitClusters],MuonSystem->metPhi);
	...
	//cout<<"DEBUG: dtRechitsCorrectX " << dtRechitsCorrectX[i] << endl;
        p.phi = dtRechitsCorrectPhi[i];
        p.eta = dtRechitsCorrectEta[i];
        p.x = dtRechitsCorrectX[i];
        p.y = dtRechitsCorrectY[i];
        p.z = dtRechitsCorrectZ[i];
        p.t = dtRechitsTime[i];
        p.twire = dtRechitsTime[i];
      ...
      for ( auto &tmp : ds_dtRechit.CscCluster ) {
        ...
        {
          if (RazorAnalyzerSUEP::deltaR(MuonSystem->cscRechitClusterEta[i],MuonSystem->cscRechitClusterPhi[i],tmp.eta, tmp.phi)<0.4) overlap = true;
        }
        if (overlap) continue;
	...
	//std::cout<<"DEBUG: clustering x value " << tmp.x << std::endl;
         ...
         MuonSystem->dtRechitClusterNoiseHit[MuonSystem->nDtRechitClusters] = MuonSystem->dtRechitClusterNoiseHitStation1[MuonSystem->nDtRechitClusters] +
          ...
          {
            if (fabs(jetEta[i]>3.0)) continue;
            if (RazorAnalyzerSUEP::deltaR(jetEta[i], jetPhi[i], MuonSystem->dtRechitClusterEta[MuonSystem->nDtRechitClusters],MuonSystem->dtRechitClusterPhi[MuonSystem->nDtRechitClusters]) < 0.4 && jetPt[i] > MuonSystem->dtRechitClusterJetVetoPt[MuonSystem->nDtRechitClusters] ) {
          ...
          for(int i = 0; i < nMuons; i++)
          {
            if (fabs(muonEta[i]>3.0)) continue;
            float muonIso = (muon_chargedIso[i] + fmax(0.0,  muon_photonIso[i] + muon_neutralHadIso[i] - 0.5*muon_pileupIso[i])) / muonPt[i];
            if (RazorAnalyzerSUEP::deltaR(muonEta[i], muonPhi[i], MuonSystem->dtRechitClusterEta[MuonSystem->nDtRechitClusters],MuonSystem->dtRechitClusterPhi[MuonSystem->nDtRechitClusters]) < 0.4 && muonPt[i] > MuonSystem->dtRechitClusterMuonVetoPt[MuonSystem->nDtRechitClusters] ) {
          ...
          for (int i = 0; i < nDtSeg; i++) {
              if (RazorAnalyzerSUEP::deltaR(dtSegEta[i], dtSegPhi[i], MuonSystem->dtRechitClusterEta[MuonSystem->nDtRechitClusters],MuonSystem->dtRechitClusterPhi[MuonSystem->nDtRechitClusters]) < 0.4) {
              ...
              if (abs(RazorAnalyzerSUEP::deltaPhi(dtSegPhi[i],MuonSystem->dtRechitClusterPhi[MuonSystem->nDtRechitClusters]))>2) {
          ...
          if (!isData)
          {
            for(int j = 0; j < MuonSystem->nGLLP;j++)
            {
	      //cout << "LB DEB: gen level matching DT ... " << endl;
              double current_delta_r = RazorAnalyzerSUEP::deltaR(tmp.eta, tmp.phi, MuonSystem->gLLP_eta[j], MuonSystem->gLLP_phi[j]);
              if (current_delta_r < min_deltaR)
              {
                min_deltaR = current_delta_r;
                index = j;
              }
	      //cout << "LB DEB: gen level matching DT done! " << endl;
	    ...
	    //cout << "**************************************************" << endl;
	    //cout << "DT Cluster n. " << MuonSystem->nDtRechitClusters << endl;
	    ...
	    //cout << "Dau idx for DT: " << d1 << " " << MuonSystem->gLLP_daughter_e[d1] << endl;
	    //cout << "Dau idx for DT: " << d2 << " " << MuonSystem->gLLP_daughter_e[d2] << endl;	    

	    //cout << "LB DEB: chosen particle index DT? " << index << endl;
             ...
             MuonSystem->dtRechitCluster_match_gLLP_dau1_e[MuonSystem->nDtRechitClusters] = MuonSystem->gLLP_daughter_e[d1];
             MuonSystem->dtRechitCluster_match_gLLP_dau2_e[MuonSystem->nDtRechitClusters] = MuonSystem->gLLP_daughter_e[d2];

	     //cout << "LB DEB: chosen particle index DT done! " << index << endl;
          ...
          for (int i = 0; i < nDtRechits; i++) {
            if (RazorAnalyzerSUEP::deltaR(dtRechitsCorrectEta[i], dtRechitsCorrectPhi[i], MuonSystem->dtRechitClusterEta[MuonSystem->nDtRechitClusters],MuonSystem->dtRechitClusterPhi[MuonSystem->nDtRechitClusters]) < 0.5 )
            {
              if (dtRechitsStation[i] == 1) MuonSystem->dtRechitCluster_match_MB1hits_0p5[MuonSystem->nDtRechitClusters] ++;
            }
            if (RazorAnalyzerSUEP::deltaR(dtRechitsCorrectEta[i], dtRechitsCorrectPhi[i], MuonSystem->dtRechitClusterEta[MuonSystem->nDtRechitClusters],MuonSystem->dtRechitClusterPhi[MuonSystem->nDtRechitClusters]) < 0.4 )
            {
              if (dtRechitsStation[i] == 1) MuonSystem->dtRechitCluster_match_MB1hits_0p4[MuonSystem->nDtRechitClusters] ++;
            }
            if(abs(dtRechitsWheel[i]-MuonSystem->dtRechitClusterWheel[MuonSystem->nDtRechitClusters])==1 && dtRechitsStation[i] == 1)
            {
              if (abs(RazorAnalyzerSUEP::deltaPhi(dtRechitsCorrectPhi[i], MuonSystem->dtRechitClusterPhi[MuonSystem->nDtRechitClusters])) < TMath::Pi()/4.0 )
              {
                if (dtRechitsWheel[i]-MuonSystem->dtRechitClusterWheel[MuonSystem->nDtRechitClusters] == 1) MuonSystem->dtRechitCluster_match_MB1hits_cosmics_plus[MuonSystem->nDtRechitClusters] ++;
         ...
         for (int i = 0; i < nRpc; i++) {
           ...
           if (abs(RazorAnalyzerSUEP::deltaPhi(rpcPhi[i], MuonSystem->dtRechitClusterPhi[MuonSystem->nDtRechitClusters])) < 0.5 )
           {
           ...
           if(RazorAnalyzerSUEP::deltaR(rpcEta[i], rpcPhi[i], MuonSystem->dtRechitClusterEta[MuonSystem->nDtRechitClusters], MuonSystem->dtRechitClusterPhi[MuonSystem->nDtRechitClusters]) < 0.4 )
           {
         ...
         for (unsigned int l = 0; l < dtRechitCluster_match_rpcBx.size(); l++)
         {
          ...
          MuonSystem->dtRechitClusterMet_dPhi[MuonSystem->nDtRechitClusters] =  RazorAnalyzerSUEP::deltaPhi(MuonSystem->dtRechitClusterPhi[MuonSystem->nDtRechitClusters],MuonSystem->metPhi);
      ...
      {
	//cout << "LB DEB Ready to fill the tree! " << endl;
	//cout << "LB DEB gLLP_eta.size() " << gLLP_eta.size() << endl;	
	MuonSystem->tree_->Fill();
	//cout << "LB DEB tre fill called! " << endl;
      }


    }

  //cout << "LB DEB after tree fill? " << endl;
	...
	//cout << "LB DEB Closing out file! " << endl;
