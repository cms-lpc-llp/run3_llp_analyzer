...
void llp_MuonSystem_CA_mdsnano_hnl::Analyze(bool isData, int options, string outputfilename, string analysisTag) {
  ...
  for (Long64_t jentry = 0; jentry < fChain->GetEntries(); jentry++) {
    ...
    int promptTau_idx = -1;
    int HNLs_found = 0;
    int promptTaus_found = 0;
    if (!isData) {
      for (int i = 0; i < nGenPart; i++) {
        // check if particle is tau or tau neutrino and mother is HNL, if so save mothder Idx
        if ((abs(GenPart_pdgId[i]) == 15 || abs(GenPart_pdgId[i]) == 16) && GenPart_pdgId[GenPart_genPartIdxMother[i]] == 9900012) {
        ...
        if (abs(GenPart_pdgId[i]) != 15 || abs(GenPart_pdgId[GenPart_genPartIdxMother[i]]) != 24)
          continue;
      ...
      bool found_final_tau = false;
      for (int i = 0; i < nGenPart; i++) {
        if (GenPart_genPartIdxMother[i] == promptTau_idx && abs(GenPart_pdgId[i]) == 15) {
          promptTau_idx = i;

          for (int j = 0; j < nGenPart; j++) {
            if (GenPart_genPartIdxMother[j] == promptTau_idx && abs(GenPart_pdgId[j]) == 16) {
              found_final_tau = true;
              break;
            }
          }
        }
      ...
      if (HNLs_found != 1 || promptTaus_found != 1) {
        cout << "Warning: found " << HNLs_found << " HNLs and " << promptTaus_found << " prompt taus in event " << eventNum << endl;
     ...
     */
      ...
      for (int i = 0; i < nGenPart; i++) {
        if (GenPart_genPartIdxMother[i] == promptTau_idx) {
      ...
      for (int j = 0; j < nGenVisTau; j++) {
        ...
        if (GenVisTau_genPartIdxMother[j] == promptTau_idx) //if there is a visTau in the output, it is necessarily from the genTau
        {
          ...
          MuonSystem->gVisTauFractionOfTotalPt[MuonSystem->nGenVisTau] = GenVisTau_pt[j] / GenPart_pt[promptTau_idx];
      ...
      /*
          ...
          if( MuonSystem->nGenVisTau > 0 && !foundMatchHadronic){
              cout<<eventNum<<" tau_idx: "<<promptTau_idx<<" vis tau mother idx: "<<GenVisTau_genPartIdxMother[0]<<endl;
    ...
    if (!HLT_CscCluster100_PNetTauhPFJet10_Loose && isData)
      continue; //UNCOMMENT WHEN WE HAVE CLUSTERS
    ...
    for (int i = 0; i < nTau; i++) {
      ...
      if (Tau_idDeepTau2018v2p5VSjet[i] >= 1 && Tau_idDeepTau2018v2p5VSe[i] >= 2 && Tau_idDeepTau2018v2p5VSmu[i] == 4)
        MuonSystem->tauIsVVVLoose[MuonSystem->nTaus] = true;
      if (Tau_idDeepTau2018v2p5VSjet[i] >= 2 && Tau_idDeepTau2018v2p5VSe[i] >= 2 && Tau_idDeepTau2018v2p5VSmu[i] == 4)
        MuonSystem->tauIsVVLoose[MuonSystem->nTaus] = true;
      if (Tau_idDeepTau2018v2p5VSjet[i] >= 3 && Tau_idDeepTau2018v2p5VSe[i] >= 2 && Tau_idDeepTau2018v2p5VSmu[i] == 4)
        MuonSystem->tauIsVLoose[MuonSystem->nTaus] = true;
      if (Tau_idDeepTau2018v2p5VSjet[i] >= 4 && Tau_idDeepTau2018v2p5VSe[i] >= 2 && Tau_idDeepTau2018v2p5VSmu[i] == 4)
        MuonSystem->tauIsLoose[MuonSystem->nTaus] = true;
      if (Tau_idDeepTau2018v2p5VSjet[i] >= 5 && Tau_idDeepTau2018v2p5VSe[i] >= 2 && Tau_idDeepTau2018v2p5VSmu[i] == 4)
        MuonSystem->tauIsMedium[MuonSystem->nTaus] = true;
      if (Tau_idDeepTau2018v2p5VSjet[i] >= 6 && Tau_idDeepTau2018v2p5VSe[i] >= 2 && Tau_idDeepTau2018v2p5VSmu[i] == 4)
        MuonSystem->tauIsTight[MuonSystem->nTaus] = true;
      if (Tau_idDeepTau2018v2p5VSjet[i] >= 7 && Tau_idDeepTau2018v2p5VSe[i] >= 2 && Tau_idDeepTau2018v2p5VSmu[i] == 4)
        MuonSystem->tauIsVTight[MuonSystem->nTaus] = true;
      if (Tau_idDeepTau2018v2p5VSjet[i] >= 8 && Tau_idDeepTau2018v2p5VSe[i] >= 2 && Tau_idDeepTau2018v2p5VSmu[i] == 4)
        MuonSystem->tauIsVVTight[MuonSystem->nTaus] = true;
      ...
      if (!isData) {
        MuonSystem->deltaR_GenTauRecoTau[MuonSystem->nTaus] = RazorAnalyzerMerged::deltaR(Tau_eta[i], Tau_phi[i], GenPart_eta[promptTau_idx], GenPart_phi[promptTau_idx]);
    ...
    if (MuonSystem->nTaus == 0 && isData)
      continue; //no taus found, skip event
