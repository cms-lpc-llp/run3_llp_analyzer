...
void llp_MuonSystem_CA_mdsnano_hnl::Analyze(bool isData, int options, string outputfilename, string analysisTag) {
  ...
  //DIFFERENT FOR HNL!!!!!!!!
  //NOTE: All signal events will be processed regardless of option, but option matters for loading gen info for MC bkg samples
  //1 = ELECTRON TYPE HNL
  //2 = MUON TYPE HNL
  //3 = TAU TYPE HNL
  //4 = WtoENu MC Bkg
  ...
  //8 = ZtoMuMu MC Bkg
  //9 = ZtoTauTau MC Bkg
  //11 = ELECTRON TYPE HNL FROM B
  //12 = MUON TYPE HNL FROM B
  //13 = TAU TYPE HNL FROM B
  ...
  //for clustering efficiency study
  TH1I* nCscRechits_matchedLLP = new TH1I("nCscRechits_matchedLLP", "nCscRechits_matched_LLP_Total", 25, 0, 300);
  TH1I* nCscRechits_inCluster = new TH1I("nCscRechits_inCluster", "nCscRechits_inCluster", 25, 0, 300);
  TH1F* fractionOfRechitsInCluster = new TH1F("fractionOfRechitsInCluster", "fractionOfRechitsInCluster", 25, 0, 1);

  TH1I* nCscRechits_noLLP = new TH1I("nCscRechits_noLLP", "nCscRechits_no_LLP_Total", 25, 0, 300);
  TH1I* nCscRechits_inCluster_noLLP = new TH1I("nCscRechits_inCluster_noLLP", "nCscRechits_inCluster_noLLP", 25, 0, 300);
  TH1F* fractionOfRechitsInCluster_noLLP = new TH1F("fractionOfRechitsInCluster_noLLP", "fractionOfRechitsInCluster_noLLP", 25, 0, 1);
  ...
  for (Long64_t jentry=0; jentry<fChain->GetEntries();jentry++) {
    ...
    bool ele_passCutBasedIDMedium[10] = {0}; // ele_passCutBasedIDTight
    bool ele_passCutBasedIDLoose[10] = {0}; // ele_passCutBasedIDTight
    bool ele_passCutBasedIDVeto[10] = {0}; // ele_passCutBasedIDVeto

    for (int i = 0; i < nElectron; i++) {
      ele_passCutBasedIDTight[i] = Electron_cutBased[i] >= 4;
      ele_passCutBasedIDMedium[i] = Electron_cutBased[i] >= 3;
      ele_passCutBasedIDLoose[i] = Electron_cutBased[i] >= 2;
    ...
    auto muonIsMedium = Muon_mediumId; // muonIsMedium
    ...
    //new code to find the index of the HNL that displaced HNL and the prompt tau
    //changed from what was here before to account for scenario with two HNLs with different status codes - only select the one that decays directly to tau or tau neturino
    int HNL_idx = -1;
    int promptLep_idx = -1;
    int HNLs_found = 0;
    int promptLeps_found = 0;
    int W_B_idx = 0;
    ...
    if (!isData && (option<4 || option>10))
    {
      //use first event to determine the channel of the HNL sample
      //if (jentry==0){
        for (int i = 0; i < nGenPart; i++){
          if (abs(GenPart_pdgId[i])<11 || abs(GenPart_pdgId[i])>16 || abs(GenPart_pdgId[i]) % 2 == 0) continue;
          for (int j = 0; j < nGenPart; j++)
          {
            if (GenPart_genPartIdxMother[j] == GenPart_genPartIdxMother[i] && GenPart_pdgId[j] == HNL_pdg)
            {
              //cout<<"found lep and HNL from same mother"<<endl;
              lepton_pdgID = abs(GenPart_pdgId[i]);
              neutrino_pdgID = abs(GenPart_pdgId[i]) + 1;
              W_B_idx = GenPart_genPartIdxMother[i];
            }
          }

        //}
        MuonSystem->W_B_pdgId = GenPart_pdgId[W_B_idx];
        MuonSystem->W_B_genPt = GenPart_pt[W_B_idx];
        MuonSystem->W_B_genEta = GenPart_eta[W_B_idx];
        MuonSystem->W_B_genPhi = GenPart_phi[W_B_idx];
        //cout<<"Prompt Lepton PDG ID: "<< lepton_pdgID<<endl;
        //cout<<"Neutrino PDG ID: "<< neutrino_pdgID<<endl;
      }
      for (int i = 0; i < nGenPart; i++)
      {
        // check if particle is tau or tau neutrino and mother is HNL, if so save mothder Idx
        if ((abs(GenPart_pdgId[i]) == lepton_pdgID || abs(GenPart_pdgId[i]) == neutrino_pdgID) && GenPart_pdgId[GenPart_genPartIdxMother[i]]==HNL_pdg) 
        {
          if (GenPart_genPartIdxMother[i]!=HNL_idx){
              HNL_idx = GenPart_genPartIdxMother[i];
              HNLs_found++;
          }
        }
        //second loop to find HNL (though maybe not the "right" one needed for the previous if statement), then we check if it and the tau have the same W or B mother
      
        //for W-HNL
        if ((option<4) && (abs(GenPart_pdgId[i]) != lepton_pdgID || abs(GenPart_pdgId[GenPart_genPartIdxMother[i]])!=24)) continue;
        //for B-HNL
        if ((option>10) && (abs(GenPart_pdgId[i]) != lepton_pdgID || (abs(GenPart_pdgId[GenPart_genPartIdxMother[i]])!=511 &&  abs(GenPart_pdgId[GenPart_genPartIdxMother[i]])!=521 && abs(GenPart_pdgId[GenPart_genPartIdxMother[i]])!=531))) continue;
        for (int j = 0; j < nGenPart; j++)
        {
          if (GenPart_genPartIdxMother[j] == GenPart_genPartIdxMother[i] && GenPart_pdgId[j] == HNL_pdg)
          {
      ...
      //cout<<"HNLs found"<<HNLs_found<<endl;
      //now, find updated pythia tau if applicable - if there is no tau whose mother is the tagged prompt tau, or if theres a daugter tau that has tau neutrino as a daugter, it is the "final" tau
      bool found_final_lep = false;
      for (int i = 0; i < nGenPart; i++)
      {
        if (GenPart_genPartIdxMother[i] == promptLep_idx && abs(GenPart_pdgId[i]) == lepton_pdgID)
        {
          promptLep_idx = i;
          
          for (int j = 0; j < nGenPart; j++)
          {
            if (GenPart_genPartIdxMother[j] == promptLep_idx && abs(GenPart_pdgId[j]) == neutrino_pdgID)
            {
              found_final_lep = true;
              break;
            }
          }
        }
        if (found_final_lep) break;
      } 
    
      if ((HNLs_found != 1 || promptLeps_found != 1) && option<4)
        {
          cout << "Warning: found " << HNLs_found << " HNLs and " << promptLeps_found << " prompt leptons in event " << eventNum << endl;
          cout << "Skipping this event, something is wrong!" << endl;
          continue;
        }
      ...
      TLorentzVector LLP = makeTLorentzVectorPtEtaPhiM(GenPart_pt[HNL_idx], GenPart_eta[HNL_idx], GenPart_phi[HNL_idx], GenPart_mass[HNL_idx]);
      MuonSystem->gLLP_eta[MuonSystem->nGLLP] = GenPart_eta[HNL_idx];
      MuonSystem->gLLP_phi[MuonSystem->nGLLP] = GenPart_phi[HNL_idx];
      MuonSystem->gLLP_pt[MuonSystem->nGLLP] = GenPart_pt[HNL_idx];
      ...
      for (int j = 0; j < nGenPart; j++) {
        if (GenPart_genPartIdxMother[j] == HNL_idx) {
          decay_x = GenPart_vx[j] - PV_x;
          decay_y = GenPart_vy[j] - PV_y;
          decay_z = GenPart_vz[j] - PV_z;
          foundDaughter = true;
          break;
        }
      }
      ...
      if (abs(MuonSystem->gLLP_decay_vertex_z[MuonSystem->nGLLP]) < 661.0 && MuonSystem->gLLP_decay_vertex_r[MuonSystem->nGLLP] < 800 && MuonSystem->gLLP_decay_vertex_r[MuonSystem->nGLLP] > 200.0)
        MuonSystem->gLLP_dt[MuonSystem->nGLLP] = true;
          ...
          //now gen taus
          /*
          MuonSystem->nGenTaus = 0;
          TLorentzVector gTau = makeTLorentzVectorPtEtaPhiM(GenPart_pt[promptLep_idx], GenPart_eta[promptLep_idx], GenPart_phi[promptLep_idx], GenPart_mass[promptLep_idx]);
          MuonSystem->gTauEta[MuonSystem->nGenTaus] = GenPart_eta[promptLep_idx];
          MuonSystem->gTauPhi[MuonSystem->nGenTaus] = GenPart_phi[promptLep_idx];
          MuonSystem->gTauPt[MuonSystem->nGenTaus] = GenPart_pt[promptLep_idx];
          MuonSystem->gTauE[MuonSystem->nGenTaus] = gTau.E();
          //MuonSystem->gTauE[MuonSystem->nGenTaus] = GenPart_pt[promptLep_idx];
          MuonSystem->gTauPdgId[MuonSystem->nGenTaus] = GenPart_pdgId[promptLep_idx];
          */
          ...
          if (lepton_pdgID==15){
            ...
            //determine decay of prompt gen tau by tagging daughter electron or muon
            //if none found, assume hadronic decay
            bool foundElectron = false; bool foundMuon = false; int lepton_decay_idx=0;
            for (int i=0; i<nGenPart; i++)
            {
              if (GenPart_genPartIdxMother[i] == promptLep_idx)
              {
                if (abs(GenPart_pdgId[i]) == 11 || abs(GenPart_pdgId[i])==13) {
                 foundElectron = (abs(GenPart_pdgId[i]) == 11); 
                 foundMuon = (abs(GenPart_pdgId[i]) == 13);
                 lepton_decay_idx = i; 
              }
            }
          }
            MuonSystem->gTauEDecay[MuonSystem->nGenTaus] = foundElectron;
            MuonSystem->gTauMuDecay[MuonSystem->nGenTaus] = foundMuon;
            MuonSystem->gTauHadronicDecay[MuonSystem->nGenTaus] = !(foundElectron || foundMuon);
            if (foundElectron || foundMuon){
              MuonSystem->nGenLeptons = 0;
              TLorentzVector gLep= makeTLorentzVectorPtEtaPhiM( GenPart_pt[lepton_decay_idx], GenPart_eta[lepton_decay_idx], GenPart_phi[lepton_decay_idx], GenPart_mass[lepton_decay_idx] );
              MuonSystem->gLepEta[MuonSystem->nGenLeptons] = GenPart_eta[lepton_decay_idx];
              MuonSystem->gLepPhi[MuonSystem->nGenLeptons] = GenPart_phi[lepton_decay_idx];
              MuonSystem->gLepPt[MuonSystem->nGenLeptons] = GenPart_pt[lepton_decay_idx];
              MuonSystem->gLepE[MuonSystem->nGenLeptons] = gLep.E();
              MuonSystem->gLepPdgId[MuonSystem->nGenLeptons] = GenPart_pdgId[lepton_decay_idx];
              MuonSystem->nGenLeptons++;
            }
            MuonSystem->nGenTaus++;
          
            //load electron/muon info for leptonically-decaying prompt tau
            

            //bool foundMatchHadronic = false;
            MuonSystem->nGenVisTau = 0;
            for (int j=0; j < nGenVisTau; j++){
              //cout<<"in vistau loop"<<endl;
              //cout<<GenPart_pdgId[GenVisTau_genPartIdxMother[j]]<<endl;
              if (GenVisTau_genPartIdxMother[j] == promptLep_idx) //if there is a visTau in the output, it is necessarily from the genTau
              {
                TLorentzVector visTau= makeTLorentzVectorPtEtaPhiM( GenVisTau_pt[j], GenVisTau_eta[j], GenVisTau_phi[j], GenVisTau_mass[j] );
                //foundMatchHadronic = true;
                MuonSystem->gVisTauDecayMode[MuonSystem->nGenVisTau] = GenVisTau_status[j];
                MuonSystem->gVisTauEta[MuonSystem->nGenVisTau] = GenVisTau_eta[j];
                MuonSystem->gVisTauPhi[MuonSystem->nGenVisTau] = GenVisTau_phi[j];
                MuonSystem->gVisTauPt[MuonSystem->nGenVisTau] = GenVisTau_pt[j];
                MuonSystem->gVisTauE[MuonSystem->nGenVisTau] = visTau.E();
                MuonSystem->gVisTauFractionOfTotalPt[MuonSystem->nGenVisTau] = GenVisTau_pt[j]/GenPart_pt[promptLep_idx];
                MuonSystem->gVisTauFractionOfTotalEnergy[MuonSystem->nGenVisTau] = visTau.E()/gTau.E();
                MuonSystem->nGenVisTau++;
                break;
              }
            }
          }
        //otherwise this is an electron or muon sample
        else{
          MuonSystem->nGenLeptons = 0;
          TLorentzVector gLep= makeTLorentzVectorPtEtaPhiM( GenPart_pt[promptLep_idx], GenPart_eta[promptLep_idx], GenPart_phi[promptLep_idx], GenPart_mass[promptLep_idx] );
          MuonSystem->gLepEta[MuonSystem->nGenLeptons] = GenPart_eta[promptLep_idx];
          MuonSystem->gLepPhi[MuonSystem->nGenLeptons] = GenPart_phi[promptLep_idx];
          MuonSystem->gLepPt[MuonSystem->nGenLeptons] = GenPart_pt[promptLep_idx];
          MuonSystem->gLepE[MuonSystem->nGenLeptons] = gLep.E();
          MuonSystem->gLepPdgId[MuonSystem->nGenLeptons] = GenPart_pdgId[promptLep_idx];
          MuonSystem->nGenLeptons++;
        }
      
    }
    if (!isData && option>=4 && option<11){
      MuonSystem->nGenLeptons = 0;
      MuonSystem->nGenTaus = 0;
      MuonSystem->nGenVisTau = 0;
      //these are MC bkgs, so just fill all gen particles
      for (int i=0; i<nGenPart; i++)
        {
          // find leptons, but only select those that who do not have a daughter that is a lepton of the same flavor (so after radiation)
          if (abs(GenPart_pdgId[i]) == 11 || abs(GenPart_pdgId[i]) == 13 || abs(GenPart_pdgId[i]) == 15)
          {
            bool hasSameFlavorDaughter = false;
            for (int j=0; j<nGenPart; j++)
            {
              if (GenPart_genPartIdxMother[j] == i && abs(GenPart_pdgId[j]) == abs(GenPart_pdgId[i]))
              {
                hasSameFlavorDaughter = true;
                break;
              }
            }
            if (hasSameFlavorDaughter) continue;

            TLorentzVector gLep= makeTLorentzVectorPtEtaPhiM( GenPart_pt[i], GenPart_eta[i], GenPart_phi[i], GenPart_mass[i] );
            if (abs(GenPart_pdgId[i]) == 11 || abs(GenPart_pdgId[i]) == 13){
              
              MuonSystem->gLepEta[MuonSystem->nGenLeptons] = GenPart_eta[i];
              MuonSystem->gLepPhi[MuonSystem->nGenLeptons] = GenPart_phi[i];
              MuonSystem->gLepPt[MuonSystem->nGenLeptons] = GenPart_pt[i];
              MuonSystem->gLepE[MuonSystem->nGenLeptons] = gLep.E();
              MuonSystem->gLepPdgId[MuonSystem->nGenLeptons] = GenPart_pdgId[i];
              MuonSystem->nGenLeptons++;
            }
            if (abs(GenPart_pdgId[i]) == 15){
              MuonSystem->gTauEta[MuonSystem->nGenTaus] = GenPart_eta[i];
              MuonSystem->gTauPhi[MuonSystem->nGenTaus] = GenPart_phi[i];
              MuonSystem->gTauPt[MuonSystem->nGenTaus] = GenPart_pt[i];
              MuonSystem->gTauE[MuonSystem->nGenTaus] = gLep.E();
              MuonSystem->gTauPdgId[MuonSystem->nGenTaus] = GenPart_pdgId[i];
              bool foundElectron = false; bool foundMuon = false; int lepton_decay_idx=0;
                  for (int j=0; j<nGenPart; j++)
              {
                if (GenPart_genPartIdxMother[j] == i)
                {
                  if (abs(GenPart_pdgId[j]) == 11 || abs(GenPart_pdgId[j])==13) {
                  foundElectron = (abs(GenPart_pdgId[j]) == 11); 
                  foundMuon = (abs(GenPart_pdgId[j]) == 13);
                  lepton_decay_idx = j; 
                }
              }
            }
            ...
            MuonSystem->gTauHadronicDecay[MuonSystem->nGenTaus] = !(foundElectron || foundMuon);
              ...
              //cout<<"in vistau loop"<<endl;
              //cout<<GenPart_pdgId[GenVisTau_genPartIdxMother[j]]<<endl;
              if (GenVisTau_genPartIdxMother[j] == i) //if there is a visTau in the output, it is necessarily from the genTau
              {
                TLorentzVector visTau= makeTLorentzVectorPtEtaPhiM( GenVisTau_pt[j], GenVisTau_eta[j], GenVisTau_phi[j], GenVisTau_mass[j] );
                //foundMatchHadronic = true;
                MuonSystem->gVisTauDecayMode[MuonSystem->nGenVisTau] = GenVisTau_status[j];
                ...
                MuonSystem->gVisTauPt[MuonSystem->nGenVisTau] = GenVisTau_pt[j];
                MuonSystem->gVisTauE[MuonSystem->nGenVisTau] = visTau.E();
                MuonSystem->gVisTauFractionOfTotalPt[MuonSystem->nGenVisTau] = GenVisTau_pt[j]/GenPart_pt[i];
                MuonSystem->gVisTauFractionOfTotalEnergy[MuonSystem->nGenVisTau] = visTau.E()/gLep.E();
                MuonSystem->nGenVisTau++;
                break;
              }
            }
    ...
    if (analysisTag == "Summer24"){
      ...
      MuonSystem->HLT_Ele30_WPTight_Gsf = HLT_Ele30_WPTight_Gsf;
      ...
      //if (!(HLT_CscCluster100_PNetTauhPFJet10_Loose || MuonSystem->HLT_CscCluster100_Ele5 || MuonSystem->HLT_CscCluster100_Mu5) && isData) continue; //UNCOMMENT WHEN WE HAVE CLUSTERS
      ...
      for( int i = 0; i < nMuons; i++ )
      {
        ...
        if(muonPt[i] < 5) continue; //change to 5, start of trigger
        ...
        tmpMuon.dZErr = Muon_dzErr[i];
        tmpMuon.dXY = Muon_dxy[i];
        tmpMuon.dXYErr = Muon_dxyErr[i];
        tmpMuon.passDZ = abs(muon_dZ[i])<0.2;
        tmpMuon.passDXY = abs(Muon_dxy[i])<0.5;
        tmpMuon.passTightId = muonIsTight[i];
        tmpMuon.passMediumId = muonIsMedium[i];
        tmpMuon.passLooseId = muonIsLoose[i];
        ...
        tmpMuon.mT = sqrt(2*muonPt[i]*PuppiMET_pt*(1-TMath::Cos(RazorAnalyzerMerged::deltaPhi(PuppiMET_phi, muonPhi[i]))));
      ...
      {
        if (!(ele_passCutBasedIDLoose[i] || Electron_mvaIso_WP90[i])) continue; //should comment out next time
        if(elePt[i] < 5) continue; //changed to 5, start of trigger
        ...
        tmpElectron.dZErr = Electron_dzErr[i];
        tmpElectron.dXY = Electron_dxy[i];
        tmpElectron.dXYErr = Electron_dxyErr[i];
        if (abs(eleEta[i])<1.479){
          tmpElectron.passDZ = abs(ele_dZ[i])<0.1;
          tmpElectron.passDXY = abs(Electron_dxy[i])<0.05;
        }
        else{
          tmpElectron.passDZ = abs(ele_dZ[i])<0.2;
          tmpElectron.passDXY = abs(Electron_dxy[i])<0.1;
        }
        tmpElectron.passLooseId = ele_passCutBasedIDLoose[i];
        tmpElectron.passMediumId = ele_passCutBasedIDMedium[i];
        tmpElectron.passTightId = ele_passCutBasedIDTight[i];
        //add standard pF ISO for cut-based electrons
        tmpElectron.passLooseIso = Electron_pfRelIso04_all[i]<0.25;
        tmpElectron.passTightIso = Electron_pfRelIso04_all[i]<0.15;
        tmpElectron.passVTightIso = Electron_pfRelIso04_all[i]<0.10;
        tmpElectron.passVVTightIso = Electron_pfRelIso04_all[i]<0.05;
        //add MVA ID electrons (which include ISO)
        tmpElectron.MVAISOScore = Electron_mvaIso[i];
        tmpElectron.passMVAISOWP80 = Electron_mvaIso_WP80[i];
        tmpElectron.passMVAISOWP90 = Electron_mvaIso_WP90[i];
        tmpElectron.mT = sqrt(2*elePt[i]*PuppiMET_pt*(1-TMath::Cos(RazorAnalyzerMerged::deltaPhi(PuppiMET_phi, elePhi[i]))));
      ...
      // if (isData && option== 2 && !(Leptons.size()>0 && (HLT_CscCluster100_Mu5 || HLT_IsoMu24))) continue;
      // if (isData && option== 1 && !(Leptons.size()>0 && (HLT_CscCluster100_Ele5 || HLT_Ele30_WPTight_Gsf))) continue;

      if (isData && option== 2 && !(Leptons.size()>0 && HLT_CscCluster100_Mu5)) continue;
      if (isData && option== 1 && !(Leptons.size()>0 && HLT_CscCluster100_Ele5)) continue;
      ...
      for ( auto &tmp : Leptons )
      {
        ...
        MuonSystem->lepDZErr[MuonSystem->nLeptons]     = tmp.dZErr;
        MuonSystem->lepDXYErr[MuonSystem->nLeptons]     = tmp.dXYErr;
        MuonSystem->lepPassDZ[MuonSystem->nLeptons]     = tmp.passDZ;
        MuonSystem->lepPassDXY[MuonSystem->nLeptons]     = tmp.passDXY;
        MuonSystem->lepTightId[MuonSystem->nLeptons] = tmp.passTightId;
        MuonSystem->lepMediumId[MuonSystem->nLeptons] = tmp.passMediumId;
        MuonSystem->lepLooseId[MuonSystem->nLeptons] = tmp.passLooseId;
        ...
        MuonSystem->lepMVAISOScore[MuonSystem->nLeptons] = tmp.MVAISOScore;
        MuonSystem->lepPassMVAISOWP80[MuonSystem->nLeptons] = tmp.passMVAISOWP80;
        MuonSystem->lepPassMVAISOWP90[MuonSystem->nLeptons] = tmp.passMVAISOWP90;
        MuonSystem->lepMt[MuonSystem->nLeptons] = tmp.mT;
        if (!isData){
            MuonSystem->deltaR_GenLepRecoLep[MuonSystem->nLeptons] = RazorAnalyzerMerged::deltaR(tmp.lepton.Eta(), tmp.lepton.Phi(), GenPart_eta[promptLep_idx], GenPart_phi[promptLep_idx]);
      }
      MuonSystem->nLeptons++;
    }
      ...
      for(int i = 0; i < nTau;  i++){
        ...
        MuonSystem->tauDxy[MuonSystem->nTaus]           = Tau_dxy[i];
        MuonSystem->tauGenPartFlav[MuonSystem->nTaus]  = Tau_genPartFlav[i];
        MuonSystem->tauDecayMode[MuonSystem->nTaus]    = Tau_decayMode[i];
        MuonSystem->tauMt[MuonSystem->nTaus] = sqrt(2*Tau_pt[i]*PuppiMET_pt*(1-TMath::Cos(RazorAnalyzerMerged::deltaPhi(PuppiMET_phi, Tau_phi[i]))));
        
        //save raw PNET discriminator scores
        MuonSystem->tauPNetVSe[MuonSystem->nTaus] = Tau_rawPNetVSe[i];
        MuonSystem->tauPNetVSjet[MuonSystem->nTaus] = Tau_rawPNetVSjet[i];
        MuonSystem->tauPNetVSmu[MuonSystem->nTaus] = Tau_rawPNetVSmu[i];
        
        //save raw UParT discriminator scores
        //not possible with version of current nano make class, so commented out for now
        /*
        MuonSystem->tauUParTVSe[MuonSystem->nTaus] = Tau_rawUParTVSe[i];
        MuonSystem->tauUParTVSjet[MuonSystem->nTaus] = Tau_rawUParTVSjet[i];
        MuonSystem->tauUParTVSmu[MuonSystem->nTaus] = Tau_rawUParTVSmu[i];
        */
        

        TLorentzVector thisTau = makeTLorentzVectorPtEtaPhiM(Tau_pt[i], Tau_eta[i], Tau_phi[i], Tau_mass[i]);
        MuonSystem->tauE[MuonSystem->nTaus]            = thisTau.E();
        float deltaRGenReco = 9999.;
        float matching_threshold = 0.4;
        if (!isData){
            MuonSystem->deltaR_GenTauRecoTau[MuonSystem->nTaus] = RazorAnalyzerMerged::deltaR(Tau_eta[i], Tau_phi[i], GenPart_eta[promptLep_idx], GenPart_phi[promptLep_idx]);
            if (MuonSystem->deltaR_GenTauRecoTau[MuonSystem->nTaus] <= matching_threshold && MuonSystem->nGenVisTau>0){ //determine pt and energy fractions of reco tau and vis tau
            MuonSystem->tauFractionOfGenVisEnergy[MuonSystem->nTaus] = thisTau.E()/MuonSystem->gVisTauE[0];
            MuonSystem->tauFractionOfGenVisPt[MuonSystem->nTaus] = thisTau.Pt()/MuonSystem->gVisTauPt[0];
        }
        }
      ...
      //if (MuonSystem->nTaus+MuonSystem->nLeptons == 0 && isData) continue; //no taus, muons, or electrons found, skip event
      if (isData && option== 3 && !(MuonSystem->nTaus>0 && HLT_CscCluster100_PNetTauhPFJet10_Loose)) continue;
      // for MC bkgs, don't save events that don't fire trigger or have no leptons/taus
      if (!isData && option>=4 && option<11 && !(MuonSystem->nTaus+MuonSystem->nLeptons>0 && (HLT_CscCluster100_PNetTauhPFJet10_Loose || HLT_CscCluster100_Ele5 || HLT_CscCluster100_Mu5 || HLT_Ele30_WPTight_Gsf || HLT_IsoMu24))) continue;  

      //found too many events for MC bkgs with the previous statement. Will make it tighter and require the cross trigger
      if (!isData && option>=4 && option<11 && !(MuonSystem->nTaus+MuonSystem->nLeptons>0 && (HLT_CscCluster100_PNetTauhPFJet10_Loose || HLT_CscCluster100_Ele5 || HLT_CscCluster100_Mu5))) continue;  
    ...
    for (int i = 0; i < nJets; i++) {
      ...
      */
      ...
      for (int i = 0; i < nJets; i++) {
        if (jetPt[i] > 50)
          continue;
        if (fabs(jetEta[i]) > 2.65 && fabs(jetEta[i]) <= 3.139) {
          MuonSystem->HTSoftJets2p65to3p139 += jetPt[i];
          MuonSystem->nSoftJets2p65to3p139++;
        }
        if (fabs(jetEta[i]) > 2.65) {
          MuonSystem->HTSoftJetsLargerThan2p65 += jetPt[i];
          MuonSystem->nSoftJetsLargerThan2p65++;
        }
        if (fabs(jetEta[i]) > 3.139) {
          MuonSystem->HTSoftJetsLargerThan3p139 += jetPt[i];
      ...
      */
      ...
      bool cluster_matchedMuon=false;
	  ...
	  // if (tmp.nCscRechitsChamberPlus11 + tmp.nCscRechitsChamberPlus12 + tmp.nCscRechitsChamberMinus11 + tmp.nCscRechitsChamberMinus12 > 0)continue;
          ...
          for(int i = 0; i < nMuons; i++)
          {
            ...
            // same computation, but with threshold of 0.8 instead of 0.4
            //fix bug here
          ...
          if(!isData)
          {
            ...
            if (MuonSystem->cscRechitCluster_match_gLLP[MuonSystem->nCscRechitClusters] = true && abs(MuonSystem->cscRechitCluster_match_gLLP_decay_z[MuonSystem->nCscRechitClusters])>400 && abs(MuonSystem->cscRechitCluster_match_gLLP_decay_z[MuonSystem->nCscRechitClusters]<1100)){
              nCscRechits_matchedLLP->Fill(ncscRechits);
              nCscRechits_inCluster->Fill(MuonSystem->cscRechitClusterSize[MuonSystem->nCscRechitClusters]);
              float ratio = (float) MuonSystem->cscRechitClusterSize[MuonSystem->nCscRechitClusters]/ncscRechits;
              fractionOfRechitsInCluster->Fill(ratio);
            }
            else{
              nCscRechits_noLLP->Fill(ncscRechits);
              nCscRechits_inCluster_noLLP->Fill(MuonSystem->cscRechitClusterSize[MuonSystem->nCscRechitClusters]);
              float ratio = (float) MuonSystem->cscRechitClusterSize[MuonSystem->nCscRechitClusters]/ncscRechits;
              fractionOfRechitsInCluster_noLLP->Fill(ratio);
            }
          ...
          //take deltaEta and deltaPhi of cluster with prompt tau - offline will require there to only be one reco tau, so here we just take differences with first tau
          ...
          //same topology variables with highest pT prompt Lepton in event
          ...
          if (MuonSystem->cscRechitClusterMuonVetoPt0p8Thresh[MuonSystem->nCscRechitClusters]>0) cluster_matchedMuon=true;
      ...
      if (MuonSystem->nCscRechitClusters_nocut == 0 && (isData || (option>=4 && option<11))) continue; 
      if (isData && option==2 && cluster_matchedMuon) continue; //too many muon+cluster events otherwise
  ...
  if (!isData) {
    ...
    // nCscRechits_matchedLLP->Write("nCscRechits_matchedLLP");
    // nCscRechits_inCluster->Write("nCscRechits_inCluster");
    // fractionOfRechitsInCluster->Write("fractionOfRechitsInCluster");
    // nCscRechits_noLLP->Write("nCscRechits_noLLP");
    // nCscRechits_inCluster_noLLP->Write("nCscRechits_inCluster_noLLP");
    // fractionOfRechitsInCluster_noLLP->Write("fractionOfRechitsInCluster_noLLP");
    outFile->Close();
  } else {
    cout << "Filled Total of " << NEvents->GetBinContent(1) << " Events\n";
    cout << "Writing output trees..." << endl;
