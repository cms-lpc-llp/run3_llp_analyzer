...
DEFAULT_SCAN_SIZE = np.arange(135, 355, 5)
DEFAULT_SCAN_DPHI = np.arange(1.6, 2.9, 0.1)


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run ABCD VR1 closure studies from the notebook as a script, "
            "saving cutflows, ABCD tables, and scan plots."
        )
    )
    parser.add_argument(
        "--cutflow",
    ...
    parser.add_argument(
        "--scan-size",
        default=None,
        help="Comma-separated list of cluster size thresholds to scan.",
    )
    parser.add_argument(
        "--scan-dphi",
        default=None,
        help="Comma-separated list of dPhi thresholds to scan.",
    )
    parser.add_argument(
        "--blind",
    ...
    parser.add_argument(
        "--outdir",
        default="ABCD_VR1_ClosureStudies_Output",
...
def is_signal_sample(name: str, cfg: Dict) -> bool:
    if "isSignal" in cfg or "is_signal" in cfg:
        return _to_bool(cfg.get("isSignal", cfg.get("is_signal")), default=False)
    return "signal" in name.strip().lower()
...
def resolve_files(sample_cfg: Dict) -> List[str]:
...
def scan_thresholds_from_branches(
    ...
    for size_cut in size_cuts:
        ...
        size_rows.append(
            {
                "size_cut": size_cut,
                "dphi_cut": baseline_dphi,
                "D_expected": D_exp,
                "D_expected_unc": D_exp_unc,
                "D_observed": D_obs,
                "D_observed_unc": D_obs_unc,
            }
        )
    ...
    return pd.DataFrame(size_rows), pd.DataFrame(dphi_rows)
...
def plot_scan(
    ...
    if show_expected and df["D_expected"].notna().any():
        plt.errorbar(
            df[x_col],
            df["D_expected"],
            yerr=df["D_expected_unc"],
            fmt="o-",
            label="Expected (ABCD)",
        )
    if show_observed and df["D_observed"].notna().any():
        plt.errorbar(
            df[x_col],
            df["D_observed"],
            yerr=df["D_observed_unc"],
            fmt="s-",
            label="Observed",
        )
    plt.xlabel(x_col)
    plt.ylabel("Events in signal bin")
    plt.title(title)
    plt.grid(True, alpha=0.3)
    if (show_expected and df["D_expected"].notna().any()) or (show_observed and df["D_observed"].notna().any()):
        plt.legend()
    plt.tight_layout()
...
def run_sample(
    ...
    dphi_cut: float,
    blind: bool,
    scan_size: List[float],
    scan_dphi: List[float],
    outdir: Path,
) -> None:
    ...
    do_scan = True
    if do_scan:
        cutflow_df, abcd_df, cumulative_events, csc_cluster_mask = Produce_Cutflow_v2.makeCutflow(
            events,
            ...
            blind=blind,
            flavor=flavor,
            sample_ctau=float(cfg.get("sample_ctau", 1000)),
    ...
    sample_dir = outdir / cutflow_tag / name
    ...
    if do_scan:
        ...
        scan_size_df.to_csv(sample_dir / "scan_clustersize.csv", index=False)
        scan_dphi_df.to_csv(sample_dir / "scan_dphi.csv", index=False)

        plot_scan(
            scan_size_df,
            "size_cut",
            sample_dir / "scan_clustersize.png",
            f"{name}: scan size cut",
            show_expected=not is_signal,
            show_observed=True,
        )
        plot_scan(
            scan_dphi_df,
            "dphi_cut",
            sample_dir / "scan_dphi.png",
            f"{name}: scan dPhi cut",
            show_expected=not is_signal,
            show_observed=True,
        )
...
def main() -> None:
    ...
    scan_size = parse_list_arg(args.scan_size, DEFAULT_SCAN_SIZE)
    scan_dphi = parse_list_arg(args.scan_dphi, DEFAULT_SCAN_DPHI)
