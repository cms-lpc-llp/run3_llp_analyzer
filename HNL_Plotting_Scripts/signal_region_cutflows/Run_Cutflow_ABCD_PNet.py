...
import Produce_Cutflow_v2_PNet
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.974,
        help="Baseline PNetVsJet cut for ABCD (default: 0.974).",
    )
    parser.add_argument(
        "--blind",
...
def _abcd_counts_from_branches(
    ...
    pnet_vals: np.ndarray,
    size_cut: float,
    pnet_cut: float,
    normalization_factor: float,
    blind: bool,
) -> Tuple[float, float, Optional[float], Optional[float]]:
    mask_a = (cluster_sizes < size_cut) & (np.abs(pnet_vals) < pnet_cut)
    mask_b = (cluster_sizes < size_cut) & (np.abs(pnet_vals) >= pnet_cut)
    mask_c = (cluster_sizes >= size_cut) & (np.abs(pnet_vals) < pnet_cut)
    mask_d = (cluster_sizes >= size_cut) & (np.abs(pnet_vals) >= pnet_cut)
...
def run_sample(
    ...
    if is_mc or float(cfg.get("signal_xsec", 1)) != 1:
        ...
        normalization_factor = Produce_Cutflow_v2_PNet.processed_lumi * float(cfg.get("signal_xsec", 1)) / num_events

    
    cutflow_df, abcd_df = Produce_Cutflow_v2_PNet.makeCutflow(
        events,
