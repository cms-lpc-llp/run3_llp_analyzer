...
python3 run_HNL_Processor.py \
  --flavor mu \
  --apply-gen-info \
  --sample root://cmseos.fnal.gov//store/group/lpclonglived/amalbert/HNL_Tau_Search/analyzer_update_021726/HNL_mu_mN_2_ctau_1000/normalized/HNL_mu_mN_2_ctau_1000_109080pb_weighted.root \
  --label "${mu_label}" \
  --is-mc true \
  ...
  --label "${tau_label}" \
  --is-mc true \
  --sample root://cmseos.fnal.gov//store/group/lpclonglived/amalbert/HNL_Tau_Search/2024_Data_mu_analyzer_update_021726/ \
  --label "${data_label}" \
  --is-mc false \
  --hists HT_SoftJets_LargerThan3p139 \
  --outdir mu_category_PT_MET_ClusterSize160_IPCuts
